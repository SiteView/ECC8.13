...
BOOL UnixCommand_Tablespace(const char *strParas, char * szReturn, int & nSize)
{
	...
	if(bRet)
	{
		float fTotalPages = atof(FindStrValue(szReturn, "SQLM_ELM_TABLESPACE_TOTAL_PAGES"));
		float fUsedPages = atof(FindStrValue(szReturn, "SQLM_ELM_TABLESPACE_USED_PAGES"));
		float fUseRate = fUsedPages*100/fTotalPages;
...
BOOL UnixCommand_Buffer(const char *strParas, char * szReturn, int & nSize)
{
	...
	if(bRet)
	{
		float fDataLReads = atof(FindStrValue(szReturn, "SQLM_ELM_POOL_DATA_L_READS"));
		float fDataPReads = atof(FindStrValue(szReturn, "SQLM_ELM_POOL_DATA_P_READS"));
		float fIndexLReads = atof(FindStrValue(szReturn, "SQLM_ELM_POOL_INDEX_L_READS"));
		float fIndexPReads = atof(FindStrValue(szReturn, "SQLM_ELM_POOL_INDEX_P_READS"));
...
BOOL UnixCommand_Cache(const char *strParas, char * szReturn, int & nSize)
{
	...
	if(bRet)
	{
		float fPkgInserts = atof(FindStrValue(szReturn, "SQLM_ELM_PKG_CACHE_INSERTS"));
		float fPkgLookups = atof(FindStrValue(szReturn, "SQLM_ELM_PKG_CACHE_LOOKUPS"));
		float fCatInserts = atof(FindStrValue(szReturn, "SQLM_ELM_CAT_CACHE_INSERTS"));
		float fCatLookups = atof(FindStrValue(szReturn, "SQLM_ELM_CAT_CACHE_LOOKUPS"));
