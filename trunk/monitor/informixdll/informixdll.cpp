...
//(CStringList& paramList, char* szReturn)
{
	...
	while(pos)
	{
		...
		{
			strDSN = strTemp.Right(strTemp.GetLength() - strlen("_DSN="));
		}

		else if(strTemp.Find("_MonitorID=") == 0)
		{
			strmid = strTemp.Right(strTemp.GetLength() - strlen("_MonitorID="));
		}
		else if(strTemp.Find("_UID=") == 0)
		{
			strUID = strTemp.Right(strTemp.GetLength() - strlen("_UID="));
		}

		else if(strTemp.Find("_PassWord=") == 0)
		{
			strPWD = strTemp.Right(strTemp.GetLength() - strlen("_PassWord="));
		}
	}
	...
	{
		sprintf(szReturn, "error=%s$", e.Description());
		return FALSE;
	}
}
...
//(CStringList& paramList, char* szReturn)
{
	...
	try{
		...
		*/
		...
		sprintf(szReturn, "nPerWait=%d$nPerNormal=%d$nconnect=%d$nwlatch=%d$nwlock=%d$nwbuff=%d$nwckpt=%d$nwlogbuf=%d$nwtrans=%d$nmonitor=%d$nincrit=%d$", nPerWait,nPerNormal,nconnect,nwlatch
				,nwlock, nwbuff, nwckpt, nwlogbuf, nwtrans,nmonitor,nincrit);
		CString strInput;
		strInput =szReturn;
		MakeCharByString(szReturn,nSize,strInput);
	}
