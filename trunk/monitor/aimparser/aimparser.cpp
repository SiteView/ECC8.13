    ...
    if (Source) {
		...
		if(buffer) {
			...
			while (1) {
				...
				{
					if(s_derive2 < s_derive) s_derive = s_derive2;
				}
...
extern "C" __declspec(dllexport) BOOL ServiceParser(const char * szSource, const int nMonitorType, char *szOut, const char* szProcName, const char *FileName)
{
	//AddToLogFile(nMonitorType, szSource);
...
extern "C" __declspec(dllexport) BOOL DisksParser(const char * szSource, const int nMonitorType, char *szOut, const char *FileName)
{
	//AddToLogFile(nMonitorType, szSource);
