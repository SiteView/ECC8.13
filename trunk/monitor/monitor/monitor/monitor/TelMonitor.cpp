...
//	hMutex=::CreateMutex(NULL,FALSE,"siteview-kkkkkkk");
	if(hMutex==NULL)
	{
		...
		sprintf(szReturn,"error=´´½¨»¥³â³ö´í!ew=%d",GetLastError());
		bResult=FALSE;
		goto err;
	}
	if(::WaitForSingleObject(hMutex,100000)==WAIT_TIMEOUT)
	{
		sprintf(szReturn,"error=²¢·¢³¬Ê±$");
		puts("²¢·¢³¬Ê±");
		bResult=FALSE;
		goto err;
	}*/
				  ...
				  char *inifile, int mtype, char *extraparam, char *custpath, char *szReturn)
{
    ...
    {
        sprintf(szReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC", 
            "BASIC_MALLOC_MEMORY_FAILE"));//·ÖÅäÄÚ´æÊ§°Ü
        return false;
    }
...
//	puts(pgd->databuffer);
	...
	//strTemp.Replace("ïïÿ?ú^_ïïecho: not found\n","");
