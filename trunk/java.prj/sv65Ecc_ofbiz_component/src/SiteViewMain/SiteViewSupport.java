...
public class SiteViewSupport
{
    ...
    public static void InitProcess()
    {
        ...
        HashMap hashmap = CopyDefaultConfig(true);
        if(hashmap == null)
        {
...
label0:
    ...
    {
        return CopyDefaultConfig(false);
    }

    public static HashMap CopyDefaultConfig( boolean flag1)
    {
        if(addOnlyIfMissing == null)
            synchronized(addOnlyIfMissingSync)
            {
                if(addOnlyIfMissing == null)
                {
                    ...
                    addOnlyIfMissing.add(new ScheduleInstancePreferences().getSettingName());
                }
            }
        
        Array defaultConfigInfoArray = null;
		try{
			String defaultConfigFileName = Platform.getRoot() + "/classes/default.config";
			defaultConfigInfoArray = FrameFile.readFromFile(defaultConfigFileName);
		}catch(Exception obj){}
        HashMap hashmap1 = null;
        HashMap hashmap = MasterConfig.getMasterConfig();
        if(hashmap == null)
            return null;
        if(defaultConfigInfoArray != null && defaultConfigInfoArray.size() > 0)
        {
            hashmap1 = (HashMap)defaultConfigInfoArray.at(0);
        }else{
	        return hashmap;
        }
        try
        {
            ...
            if(flag1)
            {
                ...
                array1.add(hashmap);
                if(APIEntity.printFile(file, array1, "_", true, true))
