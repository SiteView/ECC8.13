...
public class UpdateConfig
{
    ...
    public static void main(String args[])
    {
        ...
        try
        {
            if(!s.equals("build"))
            {
                ...
                HashMap savehashmap = new HashMap();
                s3 = updateWebserverAddress(hashmap,savehashmap);
                if(s.length() != 0){
                    hashmap.put("_autoEmail", s);
                    savehashmap.put("_autoEmail", s);
                }
                if(s1.length() != 0 && hashmap.get("_httpPort") == null){
                    hashmap.put("_httpPort", s1);
                    savehashmap.put("_httpPort", s1);
                }
                if(s2.length() != 0){
                    hashmap.put("_haInstall", s2);
                    savehashmap.put("_haInstall", s2);
                }
                if(TextUtils.getValue(hashmap, "_version").equals(""))
                    moveOldLicense(hashmap,savehashmap);
    ...
    public static String updateWebserverAddress(HashMap hashmap,HashMap savehashma)
    {
        ...
        if(s == null)
        {
            ...
            savehashma.put("_webserverAddress", s);
        }
        return s;
    }
    ...
    public static HashMap moveOldLicense(HashMap hashmap,HashMap savehashmap)
    {
        ...
        if(j == 0)
        {
            if(!TextUtils.getValue(hashmap, "_license").equals("")){
                ...
                savehashmap.put("_licenseOld", TextUtils.getValue(hashmap, "_license"));
            }
            hashmap.put("_license", "");
            hashmap.put("_installTime", "" + TextUtils.timeSeconds());
            hashmap.put("_version", s);
            savehashmap.put("_license", "");
            savehashmap.put("_installTime", "" + TextUtils.timeSeconds());
            savehashmap.put("_version", s);
            hashmap = manageConverterLink(hashmap,savehashmap);
        }
        return hashmap;
    }

    public static HashMap manageConverterLink(HashMap hashmap,HashMap savehashmap)
    {
