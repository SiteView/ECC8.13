...
public class LineReader {
    ...
    private java.io.PrintWriter getTraceStream() {
        if (traceStream == null) {
            traceStream = (java.io.PrintWriter) streamMap.get(Thread.currentThread());
        }
        return traceStream;
    }
    ...
    public static void resetTraceStream() {
        streamMap.remove(Thread.currentThread());
        com.dragonflow.Utils.LineReader.setTraceDetail(false);
    }

    private boolean isTraceDetail() {
        if (traceDetail == null) {
            Boolean boolean1 = (Boolean) detailMap.get(Thread.currentThread());
            if (boolean1 != null) {
