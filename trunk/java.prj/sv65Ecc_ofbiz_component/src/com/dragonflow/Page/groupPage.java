...
public class groupPage extends com.dragonflow.Page.CGI {
    ...
    void printForm(String operation, String groupName, String message) throws Exception {
        String s3 = "";
        String s4 = "";
        String s5 = "";
        String s6 = "";
        ...
        String s8 = "";
        String s9 = operation;
        String parent = I18N.toDefaultEncoding(request.getValue("parent"));
        if (parent.length() > 0) {
        	parent = com.dragonflow.Page.groupPage.getGroupName(parent);
        }
        ...
        if (request.getValue("_health").length() == 0) {
            outputStream.println(operation
                    + (parent.length() <= 0 ? " Group to :" : " Subgroup to : "));
        } else {
            outputStream.println(operation + ": ");
        }
        if (s12.length() > 0) {
            outputStream.println("<A HREF="
                    + CGI.getGroupDetailURL(request, groupName) + ">" + s3 + "</A>");
        } else {
        ...
        outputStream
                .println((parent.length() <= 0 ? " Group" : " Subgroup")
                        ...
                        + "\"> " + (parent.length() <= 0 ? " Group" : " Subgroup")
                        + "<p>");
        outputStream
    ...
    public static String getMonitorOptionsHTML(String groupId)
    {
    	try {
			return new groupPage().getMonitorOptionsHTML("", groupId, 5);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "";
    }
    void printAddForm(String operation, String s1) throws Exception {
        ...
        if (request.isPost()) {
            ...
            if (com.dragonflow.Utils.I18N.isI18N) {
                ...
                s1 = I18N.toDefaultEncoding(groupName);
            }
            if (groupName.length() == 0
                    && com.dragonflow.Page.treeControl.notHandled(request)) {
                printForm(operation, groupName, "a group must have a name");
            } else if (groupName.equalsIgnoreCase("siteview")
                    && com.dragonflow.Page.treeControl.notHandled(request)) {
                printForm(operation, groupName,
                ...
                if (!groupName.equals(com.dragonflow.Page.groupPage
    ...
    void printEditForm(String operation, String groupId) throws Exception {
        String s2 = com.dragonflow.Utils.I18N.toDefaultEncoding(groupId);
        ...
        if (request.isPost()) {
            ...
            try {
                ...
                if (s3.length() == 0
                        && !com.dragonflow.Page.treeControl.notHandled(request)) {
                    printForm(operation, groupId, "a group must have a name");
                } else {
    ...
    void printDeleteForm(String s, String s1) {
        String groupId = com.dragonflow.Utils.I18N.toDefaultEncoding(s1);
        com.dragonflow.SiteView.SiteViewGroup siteviewgroup = com.dragonflow.SiteView.SiteViewGroup
                .currentSiteView();
        ...
        if (request.isPost()
            ...
            try {
                ...
                //add by hailong.yi
                try {
    				GroupEntity.delete(groupId, GroupEntity.FILENAME_EXT_MG);
    			} catch (Exception e) {
    				e.printStackTrace();
    			}
                //------------------
                     	
            	com.dragonflow.SiteView.ConfigurationChanger.deleteGroup(groupId,
                        request, null);
            ...
            String s6 = com.dragonflow.Page.groupPage.getGroupName(groupId);
            String s7 = "group";
            outputStream.println("<H2>Delete Group: <A HREF="
                    + com.dragonflow.Page.CGI.getGroupDetailURL(request, groupId)
                    + ">" + s4 + "</A>");
    ...
    public void printBody() throws Exception {
        ...
        if (operation.equals("Delete")) {
            ...
            printDeleteForm(operation, groupId);
        } else if (operation.equals("Edit")) {
            if (!request.actionAllowed("_groupEdit")) {
                throw new HTTPRequestException(557);
            }
            printEditForm(operation, groupId);
        } else if (operation.equals("Add")) {
            if (!request.actionAllowed("_groupEdit")) {
                throw new HTTPRequestException(557);
            }
            printAddForm(operation, com.dragonflow.Utils.I18N.toDefaultEncoding(groupId));
        } else if (operation.equals("AddDefaultMonitors")) {
            healthAttributes(false);
        } else if (operation.equals("flipHealthLogging")) {
