...
public class alertPage extends com.dragonflow.Page.CGI {
	...
	void printTargets(String s, String s1,
		...
		if (com.dragonflow.Page.treeControl.useTree()) {
			String s6 = com.dragonflow.Utils.TextUtils.getValue(
					hashmap, "targets");
			...
			String s11 = com.dragonflow.Utils.TextUtils.getValue(
					hashmap, "targets");
			outputStream
	 ...
	 * @throws Exception
	 */
	void printForm(String opType, String groupId,
			String monitorId, String alertId, jgl.HashMap hashmap,
			String s4) throws Exception {
		String s12;
		java.util.Enumeration enumeration1;
		String s38;
		com.dragonflow.Utils.I18N.test(groupId, 1);
		String s5 = "";
		String s6 = "";
		String s7 = "";
		String s8 = "";
		...
		int i1 = -1;
		int j1 = -1;
		String alertCategory = request.getValue("category");
		...
		if (opType.equals("Add")) {
			...
			jgl.Array array;
			if (s4 == null) {
				array = findCondition(alertId);
				if (com.dragonflow.SiteView.AtomicMonitor.alertDebug) {
					System.out
							.println("AlertPage: used findCondition pieces: "
									+ array.toString() + " id: " + alertId);
				}
			} else {
		...
		String s21 = "Error";
		if (alertCategory.equals("warning")) {
			s21 = "Warning";
		} else if (alertCategory.equals("good")) {
			s21 = "OK";
		}
		String s23 = opType;
		if (s23.equals("Add")) {
		...
		outputStream.println("<H2>" + s23 + " "
				...
				+ "</H2><P>\n");
		String s24 = getPagePOST("alert", opType)
				+ "<input type=hidden name=group value=" + groupId + ">\n"
				+ "<input type=hidden name=category value=" + alertCategory + ">\n"
				+ "<input type=hidden name=monitor value=" + monitorId + ">\n"
		...
		printTargets(groupId, monitorId, s15, hashmap);
		...
		if ((action instanceof com.dragonflow.SiteView.ServerAction)
			...
			if (request.hasValue("server")) {
				...
				String s28 = com.dragonflow.Utils.TextUtils.getValue(
		...
		while (enumeration1.hasMoreElements()) {
			class1 = (Class) enumeration1.nextElement();
	...
	void printAddForm(String s) throws Exception {
		String groupId = request.getValue("group");
		String monitorId = request.getValue("monitor");
		String alertId = request.getValue("id");
		if (request.isPost()) {
			java.util.Enumeration enumeration = request.getValues("targets");
			String groupIdnew = "";
			String monitorIdNew = "";
			...
			do {
				...
				if (groupIdnew.length() == 0) {
					groupIdnew = s10;
					monitorIdNew = s9;
				} else if (!groupIdnew.equals(s10)) {
					groupIdnew = "_master";
					monitorIdNew = "_config";
					flag = true;
				} else {
			...
			String alertCond = packCondition(request, s11, hashmap);
			boolean flag1 = hashmap.size() == 0;
			if (flag1) {
				if (s.equals("Add")) {
					addCondition(groupIdnew, monitorIdNew, alertCond, s8);
				} else {
					replaceCondition(groupId, monitorId, groupIdnew, monitorIdNew, alertId, alertCond, s8);
				}
			}
			if (flag1 && com.dragonflow.Page.treeControl.notHandled(request)) {
				...
				printForm(s, groupId, monitorId, alertId, hashmap, alertCond);
			}
		} else {
			printForm(s, groupId, monitorId, alertId, new HashMap());
		}
	}
	...
	void printDisableForm(String s) throws Exception {
		if (!request.isPost()) {
			outputStream
	...
	void disableAlerts(boolean flag) throws Exception {
		jgl.Array array = getConditions();
		jgl.HashMap hashmap = new HashMap();
	...
	public static void main(String args[]) throws java.io.IOException {
		...
		*/
		String[] keyValue = "aaaa=bbbb".split("=");
	...
	jgl.Array findCondition(String alertId) throws Exception {
		com.dragonflow.Api.Alert alert = com.dragonflow.Api.Alert.getInstance()
				.getByID((new Long(alertId)).longValue());
	...
	jgl.HashMap addCondition(String groupid, String monitorId,
			String alertCond, String s3) {
		return mungeCondition(groupid, monitorId, null, alertCond, s3);
	}

	jgl.HashMap mungeCondition(String groupid, String monitorId,
			String alertId, String alertCond, String s4) {
		if (com.dragonflow.SiteView.AtomicMonitor.alertDebug) {
			com.dragonflow.Utils.TextUtils.debugPrint("group: " + groupid
					+ ", monitorID: " + monitorId + ", id: " + alertId + ", cond: " + alertCond
					+ ", includeFilter: " + s4);
		}
		jgl.HashMap hashmap = null;
		try {
			jgl.Array array = ReadGroupFrames(groupid);
			hashmap = com.dragonflow.Page.alertPage.findMonitor(array, monitorId);
			if (alertId == null) {
				alertId = Long
						.toString(com.dragonflow.ConfigurationManager.InternalIdsManager
								.getInstance().getNextSiteviewId());
			}
			String s5 = alertCond + "\t" + alertId;
			if (s4 != null) {
				s5 = s5 + "\t";
				if (s4 != null) {
					s5 = s5 + s4;
			...
			if (com.dragonflow.SiteView.AtomicMonitor.alertDebug) {
				System.out
						.println("alertPage: Attempting to replace id: " + alertId);
			}
			while (enumeration.hasMoreElements()) {
				...
				jgl.Array array2 = com.dragonflow.SiteView.Platform.split('\t',
						s6);
				if (array2.at(2).equals(alertId)) {
					if (alertCond != null) {
						array1.add(s5);
						flag = true;
						if (com.dragonflow.SiteView.AtomicMonitor.alertDebug) {
							System.out
									.println("alertPage: replacing id: " + alertId
											+ " with: " + s6);
						}
					}
				} else {
			...
			if (!flag && alertCond != null) {
				array1.add(s5);
				if (com.dragonflow.SiteView.AtomicMonitor.alertDebug) {
					System.out
							.println("alertPage: we did not replace id: " + alertId
									+ " with: " + alertCond);
				}
			}
	...
	int getErrorCount(String s) {
		...
		if (i != -1) {
			return Math.max(0, com.dragonflow.Utils.TextUtils
	 ...
	 * @throws java.io.IOException
	 */
	public jgl.Array getConditions() throws Exception {
		jgl.Array array = new Array();
	...
	void printTestForm(String s) throws Exception {
