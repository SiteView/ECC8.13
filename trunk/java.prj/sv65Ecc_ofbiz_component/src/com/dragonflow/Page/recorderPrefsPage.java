...
*/            
            ...
            Config.configPut("_impersonatingUserName", request.getValue("userName"));
            if (request.getValue("savePassword").equals("1")) {
            	Config.configPut("_impersonatingPassword", request
                        .getValue("password"));
            }
            Config.configPut("_impersonatingDomain", request.getValue("domain"));
            Config.configPut("_qtpTestRoot", request.getValue("qtpTestRoot"));
            Config.configPut("_altTestRoot", request.getValue("altTestRoot"));
...
//        boolean flag = isQTPLicensed();
    ...
    private boolean test(com.dragonflow.HTTP.HTTPRequest httprequest,
        ...
        if (flag) {
            String s3 = com.dragonflow.SiteView.Platform
                    .perfexCommand("")
                    + "-logon 1 -u "
                    + s
                    + (s1.length() <= 0 ? "" : " -p " + s1)
