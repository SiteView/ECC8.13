...
public abstract class CGI {
    ...
    public abstract void printBody() throws Exception;
    ...
    public boolean isGroup(jgl.HashMap hashmap) {
    ...
    public static String getGroupName(String groupId) {
        com.dragonflow.Utils.I18N.test(groupId, 0);
        jgl.HashMap hashmap = null;
        try {
            jgl.Array array = com.dragonflow.Page.CGI.ReadGroupFrames(groupId, null);
            java.util.Enumeration enumeration = array.elements();
            if (enumeration.hasMoreElements()) {
            ...
            System.out.println("***id: " + groupId + ", error: "
                    + ioexception);
        }
        return com.dragonflow.Page.CGI.getGroupName(hashmap, groupId);
    }
    ...
    public static String getGroupFilePath(String groupId,
        ...
        if (com.dragonflow.SiteView.Portal.isPortalID(groupId)) {
            String as[] = com.dragonflow.SiteView.Portal
                    .getIDComponents(groupId);
            s2 = as[0];
            groupId = as[1];
        }
        String s3;
        ...
        } else if (groupId.equals("_users")) {
            s3 = "/groups/users.config";
        } else {
            s3 = "/groups/" + groupId + s1;
        }
        String s4 = "";
        if (s2.length() > 0) {
    ...
    public jgl.Array ReadGroupFrames(String groupId)
            throws Exception {
        return com.dragonflow.Page.CGI.ReadGroupFrames(groupId, request);
    }

    public static jgl.Array ReadGroupFrames(String groupId,
            com.dragonflow.HTTP.HTTPRequest httprequest)
            throws Exception {
        if (httprequest != null
                && !com.dragonflow.Page.CGI.isGroupAllowedForAccount(groupId,
                        httprequest)) {
            throw new IOException("login " + httprequest.getUser()
                    + " cannot access " + groupId);
        }
        String filename = com.dragonflow.Page.CGI.getGroupFilePath(groupId,
                httprequest);
        jgl.Array array = com.dragonflow.Properties.FrameFile.readFromFile(filename);
        if (array.size() == 0) {
    ...
    void saveSettings(jgl.HashMap hashmap) throws Exception {
        if (request.isStandardAccount()) {
    ...
    public String getMonitorOptionsHTML(String s,
    ...
    public String getMonitorOptionsHTML(jgl.Array array, jgl.Array array1,
            jgl.Array array2, int i) throws Exception {
...
*/        
    ...
    public static void expandSubgroupIDs(String s, jgl.Array array,
            com.dragonflow.Page.CGI cgi) {
        com.dragonflow.Utils.I18N.test(s, 0);
        Object obj = com.dragonflow.SiteView.SiteViewGroup
                .currentSiteView();
        if (cgi != null) {
            obj = cgi.getSiteView();
        }
    ...
    public boolean isPortalServerRequest() {
        if (request != null) {
    ...
    jgl.Array readMachines(String s) throws Exception {
        jgl.Array array = new Array();
    ...
    void saveMachines(jgl.Array array, String s) throws Exception {
		Config.configDel(s);
		for (int i = 0; i < array.size(); i++) {
			jgl.HashMap hashmap1 = (jgl.HashMap) array.at(i);
			Config.configAdd(s, com.dragonflow.Utils.TextUtils
