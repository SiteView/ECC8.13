...
public class StringProperty implements PropertyFormInterface{
	...
	public String getPrivateForOfbizForm() {
		...
		outbuffer.append("<field name=\"" + getName() + "\" title=\"" + getLabel() +  "\">");
		
		if (isMultiLine) {
			if (isEditable){
				outbuffer.append("<textarea cols=\"70\" rows=\"4\" default-value=\"");
				outbuffer.append(this.value);
				outbuffer.append("\"/>");
			}else{
				outbuffer.append("<textarea cols=\"70\" rows=\"4\" default-value=\"");
				outbuffer.append(this.value);
				outbuffer.append("\"></textarea>");
			}
		}else{
			if (isPassword) {
				outbuffer.append("<password size=\"10\" default-value=\"*********\"/>");
			} else {
				outbuffer.append("<text size=\"10\" default-value=\"");
				outbuffer.append(this.value);
				outbuffer.append("\"/>");
			}
		...
		if (isPassword) {
			outbuffer.append("<field name=\"hidden" + getName() + "\">");
			outbuffer.append("<text default-value=\"");
			outbuffer.append(this.value);
			outbuffer.append("\"/>");
			outbuffer.append("<hidden/></field>");
		}
