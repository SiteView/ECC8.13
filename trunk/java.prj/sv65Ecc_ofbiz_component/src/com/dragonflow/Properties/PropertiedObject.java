...
public class PropertiedObject {
    ...
    public String getProperty(StringProperty stringproperty) throws NullPointerException {
        ...
        if (hashmap != null) {
            ...
            if (obj != null && (obj instanceof Array)) {
                ...
                for (Enumeration<?> enumeration = array.elements(); enumeration.hasMoreElements();) {
                    if (retStr.length() != 0) {
                    	retStr = retStr + stringproperty.multiLineDelimiter + " ";
                    }
                    retStr = retStr + enumeration.nextElement();
                }
        ...
        if (retStr == null && stringproperty != null) {
        	retStr = stringproperty.defaultValue;
        }
        return retStr;
    }
    ...
    public String getProperty(String key) throws NullPointerException {
        StringProperty stringproperty = getPropertyObject(key);
        if (stringproperty != null) {
            return getProperty(stringproperty);
        }

        HashMap hashmap = findTable(key);
