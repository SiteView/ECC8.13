...
//        }
    ...
    private static int getNextId(Array array, boolean flag) {
        ...
        int retNextId = 1;
        int curNextId = TextUtils.toInt(TextUtils.getValue(hashmap, SiteViewObject.pNextID.getName()));
        if (curNextId > retNextId) {
        	retNextId = curNextId;
        }
        if (!flag) {
            return retNextId;
        ...
        for (int k = 1; k < array.size(); k ++) {
            ...
            int findNextId = TextUtils.toInt((String) hashmap1.get(Monitor.pID.getName()));
            if (findNextId >= retNextId) {
            	retNextId = findNextId + 1;
            }
        }

        if (retNextId > curNextId) {
            hashmap.put(SiteViewObject.pNextID.getName(), String.valueOf(retNextId + 1));
        }
        return retNextId;
    }
