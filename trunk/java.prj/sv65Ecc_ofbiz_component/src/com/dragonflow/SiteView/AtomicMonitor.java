...
public abstract class AtomicMonitor extends Monitor implements Runnable,
    ...
    public static final String DEPENDS_ON = "_dependsOn";

    public static final String DEPENDS_CONDITION = "_dependsCondition";
    ...
    public static boolean alertDebug = false;
...
//                        || MAManager.isAttached()) {
                {
                    LogManager.log("RunMonitor",(Platform.productName + " shutting down, exceeded maxMonitorSkips limit"));
                    LogManager.log("Error",(Platform.productName + " shutting down, exceeded maxMonitorSkips limit"));
                    //SiteViewSupport.ShutdownProcess();/*delete by dingbing.xu*/
					return false;
                } else {
                    setDisabledProperties("Disabled due to Skips > " + maxMonitorSkips);
                    setProperty(pDisabledDescription,"Disabled due to Skips > " + maxMonitorSkips);
...
//                    assert false : "MonitorUpdate: Unhandled exception during monitor run";
    ...
    public void saveMonitor(jgl.HashMap hashmap) {
        try {
/*        	String id = getProperty(Monitor.pID);
        	String groupId = group.getProperty(Monitor.pID);
        	MonitorEntity.save(groupId,id, hashmap);
*/        	
    ...
    public Vector<String> getScalarValues(ScalarProperty scalarproperty,
...
//                topazMeasurements.put(string, topazmeasurement);
    ...
    static {
        ...
        pFrequency.setDisplayText(MonitorIniValueReader.getCommonValue("_frequency", MonitorIniValueReader.LABEL),
        		MonitorIniValueReader.getCommonValue("_frequency", MonitorIniValueReader.DESCRIPTION));
        pFrequency.setParameterOptions(true, 101, false);
        pErrorFrequency = new FrequencyProperty("_errorFrequency", "");
        pErrorFrequency.setDisplayText(
        		MonitorIniValueReader.getCommonValue("_errorFrequency", MonitorIniValueReader.LABEL),
        		MonitorIniValueReader.getCommonValue("_errorFrequency", MonitorIniValueReader.DESCRIPTION));
        //pErrorFrequency.setDisplayText(
...
//                        + TopazInfo.getTopazName());
        String topazName = "";
		try {
			topazName = Config.getConfig("TopazName", 1L);
		} catch (Exception e) {
			e.printStackTrace();
		}
        String notLogToTopazLabel = MonitorIniValueReader.getCommonValue("_notLogToTopaz", MonitorIniValueReader.LABEL);
        notLogToTopazLabel = notLogToTopazLabel.replace("%1",topazName);
        String notLogToTopazDescription = MonitorIniValueReader.getCommonValue("_notLogToTopaz", MonitorIniValueReader.DESCRIPTION);
        notLogToTopazDescription = notLogToTopazDescription.replace("%1",topazName);
        pNotLogToTopaz.setDisplayText(notLogToTopazLabel,
        		notLogToTopazDescription);
        ...
        pMaximumRunTime = new NumericProperty("_maxrun", "600", MonitorIniValueReader.getCommonValue("_maxrun", MonitorIniValueReader.UNIT));
        ...
        pMonitorDoneTime = new NumericProperty("monitorDoneTime", "0",
        		MonitorIniValueReader.getCommonValue("monitorDoneTime", MonitorIniValueReader.UNIT));
        ...
        if (!Platform.isSiteSeerServer()) {
            ...
            pDependsOn.setDisplayText(MonitorIniValueReader.getCommonValue(DEPENDS_ON, MonitorIniValueReader.LABEL),
            		MonitorIniValueReader.getCommonValue(DEPENDS_ON, MonitorIniValueReader.DESCRIPTION));
            ...
            pDependsCondition.setDisplayText(MonitorIniValueReader.getCommonValue(DEPENDS_CONDITION, MonitorIniValueReader.LABEL),
            		MonitorIniValueReader.getCommonValue(DEPENDS_CONDITION, MonitorIniValueReader.DESCRIPTION));

            //pDependsCondition.setDisplayText("Depends Condition",
