...
public class User extends SiteViewObject {
    ...
    private static final String USERS_FILENAME = Platform.getRoot() + "/groups/" + "users.config";

    public static HashMap accountTable = new HashMapOrdered(true);

    public static HashMap litePermissions = new HashMapOrdered(true);
    ...
    private static void registerUser(String s, HashMap hashmap, HashMap hashmap1) {
        registerUser(null, s, hashmap, hashmap1);
    }

    private static void registerUser(MonitorGroup monitorgroup, String s, HashMap hashmap, HashMap hashmap1) {
        if (s.length() > 0) {
            User user = new User();
    ...
    public static Array readUsers() {
        if (usersCache == null) {
            ...
            try {
                usersCache = FrameFile.readFromFile(USERS_FILENAME);
    ...
    public static void writeUsers(Array array) throws Exception {
        FrameFile.writeToFile(USERS_FILENAME, array);
        unloadUsers();
        loadUsers();
    }
    ...
    static {
        ...
        ldapCacheExpirationInSecs = TextUtils.readLong(siteviewgroup.getProperty("_ldapCacheExpirationInSecs"), 0);
