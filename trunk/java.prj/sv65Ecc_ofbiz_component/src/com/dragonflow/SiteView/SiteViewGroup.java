...
public class SiteViewGroup extends MonitorGroup {
	public static final String module = MonitorGroup.class.getName();
...
//			return;
            ...
            String startupScript = getSetting("_startupScript");
            if (startupScript.length() > 0) {
                LogManager.log("RunMonitor", "Running startup script " + startupScript);
                CommandLine commandline = new CommandLine();
                Array array = commandline.exec(startupScript);
...
//        SiebelCmdLineSession.closeall();
    ...
    public static void saveMonitorPID(int i) {
        ...
        try {
        	...
        	Debug.log("error writing process id: " + s,module);
        	Debug.log(exception,module);
        }
    }
    ...
    public synchronized void registerSSEELogger(String s, int i, String s1, int j, String s2) throws Exception {
        try {
            ...
            for (String filename : filenames) {
                ...
                String groupId = filename.substring(0, index);
                groupId = I18N.toDefaultEncoding(groupId);
                MonitorGroup monitorgroup = (MonitorGroup) siteviewgroup.getElementByID(groupId);
...
//        }
    ...
    public static int monitorPID() {
        ...
        try {
        	...
        	Debug.log("error reading parent process id: " + s,module);
        	Debug.log(exception,module);
        }
        return i;
    }
    ...
    public void loadSettings() {
        ...
        Machine.registerMachines(getMultipleValues(new RemoteUnixInstancePreferences().getSettingName()));
    ...
    public Array getGroupFiles() {
        ...
        if (groupsDirectory.exists() && groupsDirectory.isDirectory()) {
            ...
            //if (file.exists()) {
        	if (APIEntity.exist("master.config")){
                array.add(file);
            }
            ...
            //if (file.exists()) {
        	if (APIEntity.exist("users.config")){
                array.add(file);
            }
            ...
            */
            ...
            //add by hailong.yi
            String filenames[] = APIEntity.getMgFileNames();
            for (String filename : filenames) {
            	File newfile = new File(groupsDirectory, filename);
                array.add(newfile);
            }
        ...
        Enumeration enumeration = array.elements();
        while(enumeration.hasMoreElements()){
        	File thefile = (File)enumeration.nextElement();
        	if (!thefile.exists()) {
        		try {
        			Debug.log("File : " + thefile.getName() + " create " + (thefile.createNewFile() ? "success" : "failure"),module );
				} catch (IOException e) {
					e.printStackTrace();
				}
        	}
        	Debug.log("filename ----->" + thefile.getName(),module);
        }
        return array;
    }
