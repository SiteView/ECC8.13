...
public class WebLogic6xMonitor extends BrowsableBase {
        ...
        if (stillActive()) {
            synchronized (this) {
                ...
                if (s.equals("OK")) {
                    ...
                    if (flag) {
                        s1 = s1 + "RemoteRuntimeException - with nested exception [Start server side stack trace: OutOfMemoryError  <<no stack trace available>> End]";
                    }
                    if (i > 0) {
    ...
    private boolean filterValue(String s) {
        if (s.length() > 500 && s.indexOf("RemoteRuntimeException - with nested exception") != -1) {
            int i = s.indexOf("Start server side stack trace: OutOfMemoryError  <<no stack trace available>> End");
            if (i != -1) {
                i = s.indexOf("Start server side stack trace: OutOfMemoryError  <<no stack trace available>> End", i + 50);
                if (i != -1) {
                    int j = s.indexOf("End");
                    if (j != -1) {
