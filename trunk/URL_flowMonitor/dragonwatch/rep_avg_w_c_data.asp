...
If Len(Request.cookies("rep_trans")("id"))>0 Then
	If Len(Request.cookies("rep_city")("id"))>0 Then
		sql="exec yl_rep_avg_w_c_withid " & prof_id & "," & Request.cookies("rep_trans")("id") & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		sql="exec yl_rep_avg_w_c_bytranid " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	sql="exec yl_rep_avg_w_c " & prof_id & ",'" & startDate & "','" & endDate & "'"
End If
'response.write sql
    ...
    <td colspan="3">&nbsp; <br>
      <table border="0" cellspacing="3" cellpadding="0">
        ...
        <tr>
          ...
          <td width="287" align="right"><A class='bluelink' HREF="rep_avg_w_t.asp">图表</A></td>
...
ReDim arrLineSet(iCountCitys,6)
oldid=0
j=0
For i=0 To iRsRows
	If oldid<>CInt(arrRs(1,i)) Then
...
<table width="85%" border="0" cellspacing="1" cellpadding="3">
  <tr bgcolor="#330066" align=center>
	...
	<%
	If Len(Request.cookies("rep_trans")("id"))>0 Then
		url="rep_avg_w_c_data.asp"
	Else
		url="rep_avg_w_t_data.asp"
	End If
  ...
  <tr bgcolor="<%=arrColor(i mod 2)%>" align=center>
    <td><A HREF="rep_avg_d_c_data.asp?per=d&d=<%=a&" 0:0"%>&s=<%=a&" 0:0"%>&e=<%=a&" 23:59"%>" style="text-decoration:underline"><%=FormatDateTime(a, vbLongDate)%></A></td>
	<%
