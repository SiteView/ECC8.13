...
If Len(Request.cookies("rep_trans")("id"))=0 Then
	Server.Transfer "tran_perfor_m_c_nocolor.asp"
End If
'=========================================================
...
If Len(Request.cookies("rep_trans")("id"))>0 Then
	If Len(Request.cookies("rep_city")("id"))>0 Then ' 组
		' 计算正常
		sql="exec yl_tran_perfor_m_c_withid " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & Request.cookies("rep_city")("id") & "','" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_m_c_withid_failcount " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & Request.cookies("rep_city")("id") & "','" & startDate & "','" & endDate & "'"
	Else
		' 计算正常
		sql="exec yl_tran_perfor_m_c_tranid " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_m_c_tranid_failcount " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	' 计算正常
	sql="exec yl_tran_perfor_m_c " & prof_id & ",'" & startDate & "','" & endDate & "'"
	' 计算失败
	sql2="exec yl_tran_perfor_m_c_failcount " & prof_id & ",'" & startDate & "','" & endDate & "'"
End If
'response.write sql & "<BR>" & thisMonthLength & "<BR>"
  ...
  <tr>
    <td colspan="3">
      <table width="100%" border="1" cellspacing="0" cellpadding="0">
        <tr>
          <td width="20%" align="center">时段</td>
		  <%
			...
			For i=1 To thisMonthLength+1
				...
				%><td align="center" height="25" style="cursor:hand;"><A class='bluelink' HREF="tran_perfor_d_c.asp?<%=arrQueryString(i)%>" style="text-decoration:underline"><%=i%></A></td><%
				't=DateAdd("d",1,t)
			Next
		  %>
        </tr>
        ...
        <tr>
		<%If bShowLink Then%>
          <td width="20%" height="30" align="center"><%=arrName(i)%></td>
		<%Else%>
          <td width="20%" height="30" align="center"><A class='bluelink' style="text-decoration:underline" HREF="<%=url%>?cityid=<%=arrID(i)%>&cityname=<%=arrName(i)%>"><%=arrName(i)%></A>&nbsp;</td>
		<%End If%>
		  <%
			t=DateValue(startDate)
			For j=0 To thisMonthLength
				...
				Else
				%>
				<td onClick="location.href='tran_perfor_d_t.asp?cityid=<%=arrID(i)%>&cityname=<%=arrName(i)%>&<%=arrQueryString(j+1)%>'" bgcolor="<%=arrColor2(status)%>" align="center" style="cursor:hand;" onMouseOver="showtip(<%=status%>,this,event,'平均响应时间：<%=avgTime%> 秒，完成 <%=okCount%> 个业务流程<BR>失败 <%=failcount%> 个业务流程')" onMouseOut="hidetip()">
