...
If Len(Request.cookies("rep_city")("id"))>0 Then
	If Len(Request.cookies("rep_trans")("id"))>0 Then ' 组
		' 计算正常
		sql="exec yl_tran_perfor_y_t_withid " & prof_id & "," & Request.cookies("rep_city")("id") & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_y_t_withid_failcount " & prof_id & "," & Request.cookies("rep_city")("id") & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		' 计算正常
		sql="exec yl_tran_perfor_y_t_cityid " & prof_id & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_y_t_cityid_failcount " & prof_id & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	' 计算正常
	sql="exec yl_tran_perfor_y_t " & prof_id & ",'" & startDate & "','" & endDate & "'"
	' 计算失败
	sql2="exec yl_tran_perfor_y_t_failcount " & prof_id & ",'" & startDate & "','" & endDate & "'"
End If
        ...
        <tr>
		  ...
		  <%
			...
			For i=1 To 12
				...
				'arrQueryString(i)="s=" & theYear&"-"&i&"-01 00:00" & "&e=" & theYear&"-"&i&"-"&LastDay(i)&" 23:59"
				%><td align="center" height="25" style="cursor:hand;"><A class='bluelink' HREF="tran_perfor_m_t.asp?<%=arrQueryString(i)%>" style="text-decoration:underline"><%=i&"月"%></A></td><%
			Next
		  %>
        </tr>
		<%
        ...
        <tr>
		  ...
		  <%
			For j=0 To 11
				...
				&nbsp;<%If Len(arrTime2(i,j))>0 Then%><img src="img/error_bit.gif" onClick="failreason(<%=Request.cookies("rep_trans")("id")%>,<%=Request.cookies("rep_city")("id")%>,'<%=theyear & "-" & j+1 & "-01" & " 00:00"%>','<%=theyear & "-" & j+1 & "-" & LastDay(j+1) & " 23:59"%>')"><%Else%>&nbsp;&nbsp;<%End If%>&nbsp;
				</td>
				<%
				Else
				%>
				<td onClick="location.href='tran_perfor_m_c.asp?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>&<%=arrQueryString(j+1)%>'" bgcolor="<%=arrColor2(status)%>" align="center" style="cursor:hand;" onMouseOver="showtip(<%=status%>,this,event,'平均响应时间：<%=avgTime%> 秒，完成 <%=okCount%> 个业务流程<BR>失败 <%=failcount%> 个业务流程')" onMouseOut="hidetip()">
