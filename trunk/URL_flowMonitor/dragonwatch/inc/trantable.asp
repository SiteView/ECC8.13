...
for i=0 to iRows - 1
	for j = 0 to 4
		arrWidth(j,i) = CInt(MaxPixls * CSng(arrData(j+1,i))/(MaxOverTime+0.000001))
'		response.write MaxPixls * CSng(arrData(j+1,i))/(MaxOverTime+0.000001)
	next
next
'response.end
%>
<table width="820" border="1" cellspacing=0>
  ...
  If iRows > 0 then
    For i=0 to iRows-1
		...
		Response.write "<td width=100>&nbsp;<a class='bluelink' href='tran_analysis.asp?transid=" & arrData(9,i) & "&transname=" & arrData(0,i) & "#'>" & arrData(0,i) & "</a></td>"
		...
		for j=0 to 4
			Response.write "<A href='#' onMouseOver='return STT(" & CSng(arrData(j+1,i))*1000 & "," & j+1 & ")'"
