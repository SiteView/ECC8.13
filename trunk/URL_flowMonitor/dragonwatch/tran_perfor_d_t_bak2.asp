...
<%			
			...
			For i=0 To 23
				...
				cell(i)="<td onClick=" & Chr(34) & "location.href='tran_perfor_n_c.asp?tranid=" & rsTrans(0) & "&tranname=" & rsTrans(1) & "&" & arrQueryString(i) & "'" & Chr(34) & " align='center' style='cursor:hand;' onMouseOver=" & Chr(34) & "showtip(0,this,event,'平均响应时间：0 秒，完成 0 个业务流程<BR>失败 " & iErrs & " 个业务流程')" & Chr(34) & " onMouseOut='hidetip()'>&nbsp;</td>" & vbCrLf
				rs2.Filter=""
			Next
			...
			While Not rs.Eof
				...
				Else
					iErrs=rs2(4)
				End If
				cell(CInt(rs(2)))="<td onClick=" & Chr(34) & "location.href='tran_perfor_n_c.asp?tranid=" & rsTrans(0) & "&tranname=" & rsTrans(1) & "&" & arrQueryString(i) & "'" & Chr(34) & " bgcolor='" & arrColor2(CInt(rs(5))) & "' align='center' style='cursor:hand;' onMouseOver=" & Chr(34) & "showtip(" & CInt(rs(5)) & ",this,event,'平均响应时间：" & rs(3) & " 秒，完成 " & rs(4) & " 个业务流程<BR>失败 " & iErrs & " 个业务流程')" & Chr(34) & " onMouseOut='hidetip()'>&nbsp;</td>" & vbCrLf
				rs.MoveNext
				i=i+1
				rs2.Filter=""
			Wend
			...
			While Not rs2.Eof
				theid=CInt(rs2(2))
				cell(theid)=Left(cell(theid),Len(cell(theid))-5) & "<img src='img/error_bit.gif'>&nbsp;</td>" & vbCrLf
				rs2.MoveNext
			Wend
