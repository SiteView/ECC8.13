        ...
        <tr>
		  ...
		  <%
			...
			For i=0 To 23
				...
				%><td align="center" height="25" style="cursor:hand;" title="点击显示指定时间的所有业务流程性能表"><A class='bluelink' HREF="tran_perfor_n_c.asp?<%=arrQueryString(i)%>" style="text-decoration:underline"><%If i>9 Then%><%=i%><%Else%><%="0"&i%><%End If%></A></td><%
			Next
		  %>
        </tr>
		<%
        ...
        <tr>
		  ...
		  <%
			For j=0 To 23
				...
				'	okCount=0
				'End If
				%><td onClick="location.href='tran_perfor_n_t.asp?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>&<%=arrQueryString(j)%>'" align="center" style="cursor:hand;">&nbsp;<%=arrTime(i,j)%>&nbsp;
				</td><%
			Next
		  %>
        </tr>
