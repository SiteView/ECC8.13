...
If Len(Request.cookies("rep_trans")("id"))=0 Then
	Server.Transfer "tran_perfor_w_c_nocolor.asp"
End If
'=========================================================
...
If Len(Request.cookies("rep_trans")("id"))>0 Then
	If Len(Request.cookies("rep_city")("id"))>0 Then ' 组
		' 计算正常
		sql="exec yl_tran_perfor_w_c_withid " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & Request.cookies("rep_city")("id") & "','" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_w_c_withid_failcount " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & Request.cookies("rep_city")("id") & "','" & startDate & "','" & endDate & "'"
	Else
		' 计算正常
		sql="exec yl_tran_perfor_w_c_tranid " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_w_c_tranid_failcount " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	' 计算正常
	sql="exec yl_tran_perfor_w_c " & prof_id & ",'" & startDate & "','" & endDate & "'"
	' 计算失败
	sql2="exec yl_tran_perfor_w_c_failcount " & prof_id & ",'" & startDate & "','" & endDate & "'"
End If
        ...
        <tr>
		  ...
		  <%
			...
			For i=1 To 7
				...
				<A class='bluelink' HREF="tran_perfor_d_c.asp?<%=arrQueryString(i)%>" style="text-decoration:underline">
				<%
				Select Case i
					Case 1
						Response.Write "一"
		...
		<%End If%>
		  <%
			...
			For j=1 To 6
				...
				Else
				%>
				<td onClick="location.href='tran_perfor_d_t.asp?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>&<%=arrQueryString(j)%>'" bgcolor="<%=arrColor2(status)%>" align="center" style="cursor:hand;" onMouseOver="showtip(<%=status%>,this,event,'平均响应时间：<%=avgTime%> 秒，完成 <%=okCount%> 个业务流程<BR>失败 <%=failcount%> 个业务流程')" onMouseOut="hidetip()">
			...
			Next
				...
				Else
				%>
				<td onClick="location.href='tran_perfor_d_t.asp?cityid=<%=arrID(i)%>&cityname=<%=arrName(i)%>&<%=arrQueryString(j)%>'" bgcolor="<%=arrColor2(status)%>" align="center" style="cursor:hand;" onMouseOver="showtip(<%=status%>,this,event,'平均响应时间：<%=avgTime%> 秒，完成 <%=okCount%> 个业务流程<BR>失败 <%=failcount%> 个业务流程')" onMouseOut="hidetip()">
				&nbsp;<%If Len(arrTime2(i,j))>0 Then%><img src="img/error_bit.gif"><%Else%>&nbsp;&nbsp;<%End If%>&nbsp;
				</td>
				<%
				End If
