  ...
  <tr>
    <td colspan="3">
      <table width="100%" border="1" cellspacing="0" cellpadding="0">
        ...
        <tr>
		<%If bShowLink Then%>
          <td height="30" align="center"><%=arrName(i)%></td>
		<%Else%>
          <td height="30" align="center"><A class='bluelink' style="text-decoration:underline" HREF="<%=url%>?cityid=<%=arrID(i)%>&cityname=<%=arrName(i)%>"><%=arrName(i)%></A>&nbsp;</td>
		<%End If%>
		  <%
			Dim strDateTime
			...
			For j=0 To 59
				...
				<%If failcount>0 Then%>
				<img src="img/error_bit.gif" onClick="failreason(<%=Request.cookies("rep_trans")("id")%>,<%=arrID(i)%>,'<%=strDateTime & j%>')">
				<%Else%>
