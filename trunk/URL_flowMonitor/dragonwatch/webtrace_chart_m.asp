...
'==============================================================================================================
' 计算画表所用的数据（月）
Dim i, j, k, y, iMonthLength, arrRs(), arrErr(), sql, arrTmp, iAvg, iAvgMax, iErrMax, arrLastDay

' 计算月份有多少天
arrLastDay=Array(31,28,31,30,31,30,31,31,30,31,30,31)
y=Year(startDate)
If ((y mod 4=0) and (y mod 100<>0)) Or (y mod 400=0) Then arrLastDay(1)=29  ' 闰年
iMonthLength=arrLastDay(Month(startDate)-1)

Redim arrRs(CInt(iMonthLength)), arrErr(CInt(iMonthLength))

iAvgMax=0
iErrMax=0

For i=0 To iMonthLength
	arrErr(i)=0
Next

sql="exec yl_webtrace_m " & hostid & "," & cityid & ",'" & startDate & "','" & endDate & "'"
'response.write sql
Set rs=Server.CreateObject("adodb.recordset")
rs.open sql, cnnDragon, 1, 1   ' 计算一月内各天的平均路由时间（毫秒）
...
rs.close
sql="exec yl_webtrace_m_error " & hostid & "," & cityid & ",'" & startDate & "','" & endDate & "'"
'response.write sql
rs.open sql, cnnDragon, 1, 1   ' 计算一月内各天的错误个数
...
BottomScroll=(0,<%=iMonthLength%>);
BottomScale=(0,<%=iMonthLength%>);
