...
If Len(Request.cookies("rep_city")("id"))>0 Then
	If Len(Request.cookies("rep_trans")("id"))>0 Then ' 组
		' 计算正常
		sql="exec yl_tran_perfor_n_t_withid " & prof_id & "," & Request.cookies("rep_city")("id") & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_n_t_withid_failcount " & prof_id & "," & Request.cookies("rep_city")("id") & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		' 计算正常
		sql="exec yl_tran_perfor_n_t_cityid " & prof_id & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_n_t_cityid_failcount " & prof_id & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	' 计算正常
	sql="exec yl_tran_perfor_n_t " & prof_id & ",'" & startDate & "','" & endDate & "'"
	' 计算失败
	sql2="exec yl_tran_perfor_n_t_failcount " & prof_id & ",'" & startDate & "','" & endDate & "'"
End If
...
If rs.Eof And rs.Bof Then
	Response.Write "<h1 align=center>此时段没有数据！</h1>"
Else
  ...
  <tr>
    <td colspan="3">
      <table width="100%" border="1" cellspacing="0" cellpadding="0">
		...
		%>
        <tr>
          <td height="30" align="center"><A class='bluelink' style="text-decoration:underline" HREF="<%=url%>?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>"><%=arrName(i)%></A>&nbsp;</td>
		  <%
			Dim strDateTime
			...
			For j=0 To 59
				...
				<%If Len(arrTime2(i,j))>0 Then%>
				<%If failcount>0 Then%>
				<img src="img/error_bit.gif" onClick="failreason(<%=arrID(i)%>,'<%=strDateTime & j%>')">
				<%Else%>
				<img src="img/error_bit.gif">
				<%End If%>
				<%Else%><img src="img/error_bit2.gif"><%End If%>
				</td><%
			Next
		  %>
        </tr>
...
function failreason(tranid,dt)
{
	window.open("tran_perfor_failreason.asp?tranid="+tranid+"&dt="+dt,"",'toolbar=FALSE,resizable=1,scrollbars=2,height=260,width=700,screenx=550,screeny=300');
}
</SCRIPT>