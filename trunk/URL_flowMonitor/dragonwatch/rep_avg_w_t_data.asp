...
<table width="85%" border="0" cellspacing="1" cellpadding="3">
  ...
  <tr bgcolor="<%=arrColor(i mod 2)%>" align=center>
    <td><A HREF="rep_avg_d_data.asp?per=d&d=<%=a&" 0:0"%>&s=<%=a&" 0:0"%>&e=<%=a&" 23:59"%>" style="text-decoration:underline"><%=FormatDateTime(a, vbLongDate)%></A></td>
	<%
  ...
  ' 打印周日数据
  %>
  <tr bgcolor="<%=arrColor(1 mod 2)%>" align=center>
    <td><A HREF="rep_avg_d_data.asp?per=d&d=<%=a&" 0:0"%>&s=<%=a&" 0:0"%>&e=<%=a&" 23:59"%>" style="text-decoration:underline"><%=FormatDateTime(a, vbLongDate)%></A></td>
	<%
	For j=0 To iCountTrans
