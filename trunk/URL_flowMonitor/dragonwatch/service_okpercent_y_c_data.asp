...
If Len(Request.cookies("rep_trans")("id"))>0 Then
	If Len(Request.cookies("rep_city")("id"))>0 then
		sql="exec cq_server_okpercent_y_c_bygroup "& prof_id & "," & Request.cookies("rep_trans")("id") & ","& Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		sql="exec cq_server_okpercent_y_c_bytran "& prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
		sql="exec cq_server_okpercent_y_c "& prof_id & ",'" & startDate & "','" & endDate & "'"
End If
''response.write sql
''response.end
...
If bRecExists Then
	...
	Redim arrOKPer(iRowCount-1,3,11)
	iOldId = 0
	iCurrRow = 0
	For i=0 To iRecCount - 1
		If CInt(arrData(1,i)) <> iOldId Then
    ...
    <td colspan="3">&nbsp; <br>
      <table border="0" cellspacing="3" cellpadding="0">
        ...
        <tr>
          ...
          <td width="287" align="right"><A class='bluelink' HREF="service_okpercent_y_c.asp?showchart=1">图表</A></td>
...
<table width="85%" border="0" cellspacing="1" cellpadding="3">
  ...
  <tr bgcolor="#330066" align=center>
	...
	<%
			If Len(Request.cookies("rep_city")("id"))>0 Then
				url="service_okpercent_y_c_data.asp"
			Else
				url="service_okpercent_y_t_data.asp"
			End If
    ...
    <%
		...
		For n=0 to 11
			...
			Response.Write "<td><a href="&Chr(34)&"javascript:goLink("&n+1&","&arrLastDay(n)&")"&Chr(34)&">"&Year(StartDate)&"年"&n+1&"月</td>"
...
function goLink(m,d)
{
	...
	var t=parent.frames.conditionFrame.document.all.selDay.value
	parent.frames.conditionFrame.document.all.selPer.value="m"
	if(t<d)
	{
	location.href="service_okpercent_m_c_data.asp?per=m&d="+y+"-"+m+"-"+t+" 0:0&s="+y+"-"+m+"-"+1+" 0:0&e="+y+"-"+m+"-"+d+" 0:0"
	}
	else
	{
	location.href="service_okpercent_m_c_data.asp?per=m&d="+y+"-"+m+"-"+d+" 0:0&s="+y+"-"+m+"-"+1+" 0:0&e="+y+"-"+m+"-"+d+" 0:0"
	}
}
//-->
</SCRIPT>
