...
If Len(Request.cookies("rep_trans")("id"))>0 Then
	If Len(Request.cookies("rep_city")("id"))>0 Then
		sql="exec yl_rep_avg_d_c_withid " & prof_id & "," & Request.cookies("rep_trans")("id") & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		sql="exec yl_rep_avg_d_c_bytranid " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	sql="exec yl_rep_avg_d_c " & prof_id & ",'" & startDate & "','" & endDate & "'"
End If
'response.write sql
    ...
    <td colspan="3">&nbsp; <br>
      <table border="0" cellspacing="3" cellpadding="0">
        ...
        <tr>
          ...
          <td width="287" align="right"><A class='bluelink' HREF="rep_avg_d_c.asp">图表</A></td>
...
ReDim arrLineSet(iCountCitys,23)
oldid=0
j=0
For i=0 To iRsRows
	If oldid<>CInt(arrRs(1,i)) Then
...
<table width="85%" border="0" cellspacing="1" cellpadding="3">
  <tr bgcolor="#330066" align=center>
    <td><font color="#ffffff">时段</font></td>
	<%
	If Len(Request.cookies("rep_trans")("id"))>0 Then
...
function goLink(h)
{
	...
	location.href="rep_avg_n_c_data.asp?per=h&d="+y+"-"+m+"-"+d+" "+h+":00&s="+y+"-"+m+"-"+d+" "+h+":00";
}
