        ...
        <tr>
		  ...
		  <%
			...
			For i=xStart To xEnd
				...
				%><td align="center" height="25" style="cursor:hand;" title="进入星期－城市数据表"><A HREF="tran_perfor_w_c.asp?<%=arrQueryString(xEnd-i)%>" style="text-decoration:underline"><%=Month(t)&"/"&Day(t)%></td></A><%
				t=DateAdd("d",7,t)
			Next
		  %>
        </tr>
        ...
        <tr>
		  ...
		  <%
			For j=xStart To xEnd
				...
				'	okCount=0
				'End If
				%><td onClick="location.href='tran_perfor_w_t.asp?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>&<%=arrQueryString(xEnd-j)%>'" align="center" style="cursor:hand;">&nbsp;<%=arrTime(i,j)%>&nbsp;
				</td><%
			Next
		  %>
        </tr>
