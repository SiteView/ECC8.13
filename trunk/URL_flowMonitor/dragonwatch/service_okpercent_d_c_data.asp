...
If Len(Request.cookies("rep_trans")("id"))>0 Then
	If Len(Request.cookies("rep_city")("id"))>0 then
		sql="exec cq_server_okpercent_d_c_bygroup "& prof_id & "," & Request.cookies("rep_trans")("id") & ","& Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		sql="exec cq_server_okpercent_d_c_bytran "& prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	sql="exec cq_server_okpercent_d_c "& prof_id & ",'" & startDate & "','" & endDate & "'"
End If
''response.write sql
''response.end
    ...
    <td colspan="3">&nbsp; <br>
      <table border="0" cellspacing="3" cellpadding="0">
        ...
        <tr>
          ...
          <td width="287" align="right"><A class='bluelink' HREF="service_okpercent_d_c.asp?showchart=1">图表</A></td>
...
<table width="85%" border="0" cellspacing="1" cellpadding="3">
  ...
  <tr bgcolor="#330066" align=center>
	...
	<%
			If Len(Request.cookies("rep_city")("id"))>0 Then
				url="service_okpercent_d_c_data.asp"
			Else
				url="service_okpercent_d_data.asp"
			End If
	...
	For i=0 to iRowCount-1
		...
		Else
			Response.Write "<td align='center'><a class='bluelink' href="&url&"?selBy=t&city_id="& arrOKPer(i,1,0) &"&city_name="&arrOKPer(i,0,0)&"><font color='#ffffff'>"&arrOKPer(i,0,0)&"</font></td>"
		End if
	Next
	%>
  </tr>
    ...
    <%
		...
		For n=0 to 23
			...
			Response.Write "<td><a href="&Chr(34)&"javascript:goLink("&n&")"&Chr(34)&">"&FormatDateTime(DateValue(x(0)), vbLongDate)&" "&n&":00</a></td>"
...
function goLink(h)
{
	...
	location.href="service_okpercent_n_t_data.asp?per=h&d="+y+"-"+m+"-"+d+" "+h+":00&s="+y+"-"+m+"-"+d+" "+h+":00";
}
