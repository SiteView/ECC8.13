...
If bFilteredTran Then
	If bFilteredCity Then 'group
		sql = "exec cq_tran_failreason_onegroup " & prof_id & "," & Trans_Id & "," & City_Id & ",'" & startDate & "','" & endDate & "'"
	Else
		sql = "exec cq_tran_failreason_onetran " & prof_id & "," & Trans_Id & ",'" & startDate & "','" & endDate & "'"
	End if
Else
...
''	strCity_Name = Request.cookies("rep_city")("name")
else
	...
	While Not rs.Eof
		For i=0 to 100
			If ISNULL(arrTran(i,0)) OR ISEMPTY(arrTran(i,0)) OR arrTran(i,0) = 0 then '添加
				arrTran(i,0) = rs("city_id")
				arrTran(i,1) = rs("cityname")
				exit for
			Else
				if arrTran(i,0) = rs("city_id") then Exit for
			End if
		Next
		rs.MoveNext
	Wend
  ...
  <tr valign="top">
    <td colspan="3">
      <table width="820" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          <td width="421"><b>&nbsp;&nbsp;当前预定义文件：<%=Request.cookies("rep_prf")("name")%><br>
            &nbsp; 
            ...
            <table border="0" cellspacing="0" cellpadding="0" width="399" align="right">
              <tr> 
                <td> 
                  <%
				If bFilteredCity And bFilteredTran Then
					response.write Request.cookies("rep_city")("name")
				Else
					...
					For i=0 to 100
						...
						response.write "<tr><td>&nbsp;<a class= 'bluelink' href='tran_failReason.asp?city_id=" & arrTran(i,0) & "&city_name=" & arrTran(i,1) & "'>" & arrTran(i,1) & "</a></td></tr>"
					Next
				End If
				%>
                </td>
