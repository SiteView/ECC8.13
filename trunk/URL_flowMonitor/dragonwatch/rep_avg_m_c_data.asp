...
If Len(Request.cookies("rep_trans")("id"))>0 Then
	If Len(Request.cookies("rep_city")("id"))>0 Then
		sql="exec yl_rep_avg_m_c_withid " & prof_id & "," & Request.cookies("rep_trans")("id") & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		sql="exec yl_rep_avg_m_c_bytranid " & prof_id & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	sql="exec yl_rep_avg_m_c " & prof_id & ",'" & startDate & "','" & endDate & "'"
End If
'response.write sql
...
ReDim arrLineSet(iCountCitys,thisMonthLength)
oldid=0
j=0
For i=0 To iRsRows
	If oldid<>CInt(arrRs(1,i)) Then
...
function goLink(d)
{
	...
	location.href="rep_avg_d_c_data.asp?per=d&d="+y+"-"+m+"-"+d+" 0:00&s="+y+"-"+m+"-"+d+" 0:00&e="+y+"-"+m+"-"+d+" 23:59";
}
//-->
</SCRIPT>
