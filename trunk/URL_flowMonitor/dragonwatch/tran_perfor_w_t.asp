...
If Len(Request.cookies("rep_city")("id"))>0 Then
	If Len(Request.cookies("rep_trans")("id"))>0 Then ' 组
		' 计算正常
		sql="exec yl_tran_perfor_w_t_withid " & prof_id & "," & Request.cookies("rep_city")("id") & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_w_t_withid_failcount " & prof_id & "," & Request.cookies("rep_city")("id") & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		' 计算正常
		sql="exec yl_tran_perfor_w_t_cityid " & prof_id & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
		' 计算失败
		sql2="exec yl_tran_perfor_w_t_cityid_failcount " & prof_id & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	' 计算正常
	sql="exec yl_tran_perfor_w_t " & prof_id & ",'" & startDate & "','" & endDate & "'"
	' 计算失败
	sql2="exec yl_tran_perfor_w_t_failcount " & prof_id & ",'" & startDate & "','" & endDate & "'"
End If
      ...
      <table width="100%" border="1" cellspacing="0" cellpadding="0">
        <tr>
		  ...
		  <%
			...
			Redim arrQueryString(7)
			t=DateValue(Request.cookies("rep_datetime")("start"))
			For i=1 To 7
				arrQueryString(i)="per=d&d=" & t & " 00:00&s=" & t & " 00:00" & "&e=" & t & " 23:59"
				%><td align="center" height="25" style="cursor:hand;" title="<%=t%>">
				<A class='bluelink' HREF="tran_perfor_d_t.asp?<%=arrQueryString(i)%>" style="text-decoration:underline"><!-- 星期 -->
				<%
				Select Case i
					Case 1
						Response.Write "一"
		...
		ReDim arrTime(iCount,6)
		ReDim arrTime2(iCount,6)
		ReDim arrCount(iCount,6)
		Redim arrCount2(iCount,6)
		ReDim arrStatus(iCount,6)
		...
		If Len(Request.cookies("rep_city")("id"))>0 Then ' 如果有transid则调用自己
			url="tran_perfor_w_t.asp"
		Else
			url="tran_perfor_w_c.asp"
		End If
		For i=0 To iCount
		%>
        <tr>
          <td width="15%" height="30" align="center"><A class='bluelink' style="text-decoration:underline" HREF="<%=url%>?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>"><%=arrName(i)%></A>&nbsp;</td>
		  <%
			For j=1 To 6
				...
				Else
				%>
				<td onClick="location.href='tran_perfor_d_c.asp?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>&<%=arrQueryString(j)%>'" bgcolor="<%=arrColor2(status)%>" align="center" style="cursor:hand;" onMouseOver="showtip(<%=status%>,this,event,'平均响应时间：<%=avgTime%> 秒，完成 <%=okCount%> 个业务流程<BR>失败 <%=failcount%> 个业务流程')" onMouseOut="hidetip()">
			...
			Next
				...
				Else
				%>
				<td onClick="location.href='tran_perfor_d_c.asp?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>&<%=arrQueryString(7)%>'" bgcolor="<%=arrColor2(status)%>" align="center" style="cursor:hand;" onMouseOver="showtip(<%=status%>,this,event,'平均响应时间：<%=avgTime%> 秒，完成 <%=okCount%> 个业务流程<BR>失败 <%=failcount%> 个业务流程')" onMouseOut="hidetip()">
				&nbsp;<%If Len(arrTime2(i,j))>0 Then%><img src="img/error_bit.gif"><%Else%>&nbsp;&nbsp;<%End If%>&nbsp;
				</td>
				<%
				End If
...
<SCRIPT LANGUAGE="JavaScript">parent.frames.conditionFrame.document.all.selPer.value="w";</SCRIPT>
