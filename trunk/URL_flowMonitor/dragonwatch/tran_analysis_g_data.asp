...
If bFilteredTran And bFilteredCity then  '以组的方式显示
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr valign="top">
	...
	<%
	'**************************************************************************************
	'如果用户没有选择特定的业务流程，并且分组方式不是By Transaction,提醒用户只有选择特定的*
	'Transaction才能看到颜色码，如果有过滤条件，则显示它                                  *
	'**************************************************************************************
	if bFilteredTran Or bFilteredCity then '有过滤条件
	else
		response.write "<p><b>提示:</b>要查看性能颜色标识,您必须指定一个业务流程.</p>"
	end if
	'***************************************************************************************
	%>
        ...
        <tr>
          <td width="840">
            <table cellspacing=1 cellpadding=3 border=0 bgcolor='#ffffff' width='100%'>
              ...
              <%
			if iRows > 0 then
				if bFilteredTran And bFilteredCity then  '只显示指定的业务流程
					'****************************************************************
					...
					For i=0 to iRows-1
						...
						Else
							sTag = "N/A"
							response.write "<tr bgcolor='" & bgc & "'>"
							response.write "<td>&nbsp;" & arrData(0,i) & "</td>" 'City Name
							for j=1 to 8
								if j=5 Or j=6 Or j=8 then
									sTag = 0
								else
									sTag = "N/A"
								end if
								response.write "<td>&nbsp;" & sTag & "</td>"
							next
