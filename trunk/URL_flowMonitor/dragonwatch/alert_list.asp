...
If Len(Trim(Request.Form("alertname")))>0 And Len(Request.Form("edit"))>0 Then
	...
	'Response.Write sql & "<HR>" ' 更新 alert 表的SQL
	cnnDragon.Execute sql
	...
	End Select
	sql=sql & " where alert_id=" & Request.Form("alertid")
	'Response.Write sql & "<HR>" ' 更新 alertplan 表的SQL
	cnnDragon.Execute sql
End If
'=============================================
' 更新ALERT（结束）
...
If Len(Trim(Request.Form("alertname")))>0 And Len(Request.Form("edit"))=0 Then
	...
	'response.write sql & "<HR>" ' 存入alert表的的SQL语句

	Dim alert_id
	cnnDragon.Execute sql

	Set rs=cnnDragon.Execute("select @@IDENTITY as nowid") ' 取刚INSERT后的alert_id值
	'Response.write rs(0)
	If IsNull(rs(0)) Then
		alert_id=0
	Else
	...
	End Select
	sql=sql & ",getdate())"
	cnnDragon.Execute sql  ' 保存到 alertplan 表
End If
'=============================================
' 保存新ALERT（结束）
'=============================================
...
Set rs=cnnDragon.Execute(sql)  ' 取指定 alert_id 的报警信息
If rs.Eof And rs.Bof Then	
	Response.Write "<CENTER>没有设置警报！</CENTER><BR><CENTER><A HREF='alert_add.asp'>添加新警报</A></CENTER>"
	Response.End
Else
%>
<html>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr> 
    ...
    <td width="99%" bgcolor="#CCCCCC" height="2" align="center" valign="middle"> 
      <table width="100%" border="0" cellspacing="0" cellpadding="0" height="21">
        <tr>
          <td width="58%"><b>报警列表</b></td>
          <td width="42%" align="right"><!-- #include file="inc/alertmenu.asp" -->　〖<A HREF="javascript:delselected()">删除选中的报警</A>〗</td>
        </tr>
      </table>
  ...
  <tr align="center"> 
    <td colspan="2"><br>
	  <TABLE width="100%" border="0" cellspacing="0" cellpadding="0">
	  <TR><TD align=right><INPUT TYPE="checkbox" NAME="ckAll" onClick="fnckAll()">选择全部</TD></TR>
	  </TABLE>
      <table border="0" cellspacing="1" cellpadding="3" width="100%">
        <tr bgcolor="#330066" align="center"> 
            <td width="16%" height="20"><font color="#FFFFFF"><IMG SRC="img/ic_name.gif" WIDTH="15" HEIGHT="17" BORDER=0 ALT="">警报名称</font></td>
            <td width="11%" height="20"><font color="#FFFFFF"><IMG SRC="img/ic_sevr.gif" WIDTH="18" HEIGHT="18" BORDER=0 ALT="">警报级别</font></td>
            <td width="13%" height="20"><font color="#FFFFFF"><IMG SRC="img/ic_acton.gif" WIDTH="15" HEIGHT="18" BORDER=0 ALT="">警报动作描述</font></td>
            <td width="50%" height="20"><font color="#FFFFFF"><IMG SRC="img/ic_mess.gif" WIDTH="18" HEIGHT="13" BORDER=0 ALT="">警报描述</font></td>
          <td width="5%" height="20"><font color="#FFFFFF">编辑</font></td>
          <td width="5%" height="20"><font color="#FFFFFF">删除</font></td>
        </tr>
        <%
		Dim i
		i=0
        ...
        <tr bgcolor="<%=arrColor(i mod 2)%>"> 
          ...
          <td width="5%" align="center"><a href="alert_edit.asp?alertid=<%=rs("alert_id")%>">编辑</a></td>
