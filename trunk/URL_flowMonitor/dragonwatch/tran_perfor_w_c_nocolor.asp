        ...
        <tr>
		  ...
		  <%
			For j=1 To 6
				...
				'	okCount=0
				'End If
				%><td onClick="location.href='tran_perfor_d_t.asp?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>&<%=arrQueryString(j)%>'" align="center">&nbsp;<%=arrTime(i,j)%>&nbsp;
				</td><%
			Next
				j=0
				'If Len(arrCount2(i,j))>0 Then
				...
				'End If
		  %>
				<td onClick="location.href='tran_perfor_d_c.asp?tranid=<%=arrID(i)%>&tranname=<%=arrName(i)%>&<%=arrQueryString(7)%>'" align="center">&nbsp;<%=arrTime(i,j)%>&nbsp;
				</td>
			</tr>
		<%Next%>
      </table>
