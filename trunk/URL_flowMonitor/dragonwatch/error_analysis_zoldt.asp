    ...
    <td width="97%" bgcolor="#CCCCCC"> 
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          ...
          <td align="right" width="50%" height="22"><B><%=Request.cookies("rep_trans")("name")%></B>&nbsp;</td>
        </tr>
      </table>
    </td>
  </tr>
...
If CInt(Request.QueryString("code"))=200 Then	
	sql="exec yl_error_tranid_groupcity_200 " & prof_id & "," & Request.cookies("rep_trans")("id") & ",200,'" & startDate & "','" & endDate & "'"
Else
	sql="exec yl_error_tranid_groupcity " & prof_id & "," & Request.cookies("rep_trans")("id") & "," & Request.QueryString("code") & ",'" & startDate & "','" & endDate & "'"
End If
