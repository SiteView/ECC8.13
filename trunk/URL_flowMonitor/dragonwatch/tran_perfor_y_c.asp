...
If Len(Request.cookies("rep_trans")("id"))=0 Then
	Server.Transfer "tran_perfor_y_c_nocolor.asp"
End If
'=========================================================
        ...
        <tr>
		  ...
		  <%
			...
			For i=1 To 12
				...
				%><td align="center" height="25" style="cursor:hand;" title="^0^"><A class='bluelink' HREF="tran_perfor_m_c.asp?<%=arrQueryString(i)%>" style="text-decoration:underline"><%=i&"月"%></A></td><%
			Next
		  %>
        </tr>
		<%
		...
		<%End If%>
		  <%
			For j=0 To 11
				...
				Else
				%>
				<td onClick="location.href='tran_perfor_m_t.asp?cityid=<%=arrID(i)%>&cityname=<%=arrName(i)%>&<%=arrQueryString(j+1)%>'" bgcolor="<%=arrColor2(status)%>" align="center" style="cursor:hand;" onMouseOver="showtip(<%=status%>,this,event,'平均响应时间：<%=avgTime%> 秒，完成 <%=okCount%> 个业务流程<BR>失败 <%=failcount%> 个业务流程')" onMouseOut="hidetip()">
