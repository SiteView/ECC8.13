...
function getClientHeight(){
	var screenHeight = (ns4)?window.innerHeight : document.body.clientHeight;
	return screenHeight;
}
function getClientWidth(){
	var screenWidth = (ns4)?window.innerWidth : document.body.clientWidth;
	return screenWidth;
}
function getOverDivHeight(){
	var overDivHeight=(ns4)?over.document.height : overDiv.clientHeight;
	return overDivHeight;
}
function getOverDivWidth(){
	var overDivWidth=(ns4)?over.document.width : overDiv.clientWidth;
	return overDivWidth;
}
...
function disp() {
	if ( (ns4) || (ie4) ||(ie5) ) {
		if (snow == 0) 	{
			...
			goUp=0;
			if (y-scrollTop >= getClientHeight()/2){goUp=getOverDivHeight()+offsety;}
			else{goUp=0;}
			if (x-scrollLeft > getClientWidth()/2){dir=0}else{dir=1}
			
			if (dir == 2) { // Center
				moveTo(over,x+offsetx-(width/2),y+offsety-goUp);
			}
			if (dir == 1) { // Right
				moveTo(over,x+offsetx,y+offsety-goUp);
			}
			if (dir == 0) { // Left
				moveTo(over,x-offsetx-width,y+offsety-goUp);
			}
			overDivTimerID=window.setTimeout("checkDelay()",tooltipDelay)
			//showObject(over);
			snow = 1;
		}
	}
...
function mouseMove(e) {
	...
	if (snow) {
		...
		//window.status="y=" + y+" x=" + x +"Cwidth=" + getClientWidth() +" Owidth=" +getOverDivWidth()+" OHeight=" + getOverDivHeight();
		if (x-scrollLeft > getClientWidth()/2){dir=0}else{dir=1}
		if (dir == 2) { // CenterTop
			moveTo(over,x+offsetx-(width/2),y+offsety-goUp);
		}
