...
If Len(Request.cookies("rep_city")("id"))>0 Then
	If Len(Request.cookies("rep_trans")("id"))>0 Then
		sql="exec yl_rep_avg_y_withid " & prof_id & "," & Request.cookies("rep_city")("id") & "," & Request.cookies("rep_trans")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		sql="exec yl_rep_avg_y_bycityid " & prof_id & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	sql="exec yl_rep_avg_y " & prof_id & ",'" & startDate & "','" & endDate & "'"
End If
'response.write sql
...
Else
	...
	Dim strTrans_id ' 存业务流程编号
	Dim strTrans_name ' 存业务流程名称 （以上两个）
	iMax=0 ' 最大数值
	...
	iCountTrans=UBound(arrTrans_id)-1 ' 此预定义文件包括多少个业务流程
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr valign="top">
      ...
      <table border="0" cellspacing="3" cellpadding="0">
        <tr>
          ...
          <td rowspan="3" width="187" align="left" valign="top" id=trans>
			...
			%>
			<tr>
				<td><A class='bluelink' HREF="<%=url%>?selby=c&trans_id=<%=arrTrans_id(i)%>&trans_name=<%=arrTrans_name(i)%>"><%=arrTrans_name(i)%></A></td>
			</tr>
			<%
				End If
			Next
        ...
        <tr>
          ...
          <td width="287" align="right"><A class='bluelink' HREF="rep_avg_y_t_data.asp">数据表</A></td>
	...
	ReDim arrLineSet(iCountTrans,11)
	oldid=0
	j=0
	For i=0 To iRsRows
		If oldid<>CInt(arrRs(1,i)) Then
...
BottomLabels	="1月","2月","3月","4月","5月","6月","7月","8月","9月","10月","11月","12月";
