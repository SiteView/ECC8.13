...
If Len(Request.cookies("rep_city")("id"))>0 Then
	If Len(Request.cookies("rep_trans")("id"))>0 then
		sql="exec cq_server_okpercent_w_t_bygroup "& prof_id & "," & Request.cookies("rep_trans")("id") & ","& Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	Else
		sql="exec cq_server_okpercent_w_t_bycity "& prof_id & "," & Request.cookies("rep_city")("id") & ",'" & startDate & "','" & endDate & "'"
	End If
Else
	sql="exec cq_server_okpercent_w_t "& prof_id & ",'" & startDate & "','" & endDate & "'"
End If
''response.write sql
''response.end
    ...
    <td colspan="3">&nbsp; <br>
      <table border="0" cellspacing="3" cellpadding="0">
        ...
        <tr>
          ...
          <td width="287" align="right"><A class='bluelink' HREF="service_okpercent_w_t.asp?showchart=1">图表</A></td>
...
<table width="85%" border="0" cellspacing="1" cellpadding="3">
  ...
  <tr bgcolor="#330066" align=center>
	...
	<%
		If Len(Request.cookies("rep_trans")("id"))>0 Then
		url="service_okpercent_w_t_data.asp"
	Else
		url="service_okpercent_w_c_data.asp"
	End If
    ...
    <%
		For n=1 to 6
			...
			Response.Write "<td><a href="&Chr(34)&"javascript:goLink("&Day(DateAdd("d",n-1,DateValue(x(0))))&","&Month(DateAdd("d",n-1,DateValue(x(0))))&")"&Chr(34)&">"&FormatDateTime(DateAdd("d",n-1,DateValue(x(0))), vbLongDate)&"</td>"
		...
		Response.Write "<tr bgcolor='"&arrColor(1)&"' align='center'>"
		Response.Write "<td><a href="&Chr(34)&"javascript:goLink("&Day(DateAdd("d",n-1,DateValue(x(0))))&","&Month(DateAdd("d",n-1,DateValue(x(0))))&")"&Chr(34)&">"&FormatDateTime(DateAdd("d",6,DateValue(x(0))), vbLongDate)&"</td>"

		For j=0 to iRowCount-1
		If IsEmpty(arrOKPer(j,2,0)) Then
			Response.Write("<td>&nbsp;</td>")
		Else
			Response.Write "<td>"& arrOKPer(j,3,0) &"%</td>"
		End If
		Next
...
function goLink(n,m)
{
	var y='<%=Year(FormatDateTime(DateAdd("d",n-1,DateValue(x(0))), vbLongDate))%>';
	parent.frames.conditionFrame.document.all.selPer.value="d";
	location.href="service_okpercent_d_data.asp?per=d&d="+y+"-"+m+"-"+n+" 0:0&s="+y+"-"+m+"-"+n+" 0:0&e="+y+"-"+m+"-"+n+" 23:59"
}
parent.frames.conditionFrame.document.frmCondition.selPer.value="w";
