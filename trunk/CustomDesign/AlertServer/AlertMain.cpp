...
bool WebSmsTest(string phoneNumber, string content)
{
	...
	string filename=GetSiteViewRootPath();
	filename+="\\data\\tmpinifile\\smstestresult.xml";
	WriteXML("websms",key,context,filename);
	return ret;	
}
...
bool CAlertMain::InitSerialPort()
{
	...
	strCOM += Value.c_str();
	//读取发送信息的长度
	string sendlength="60";
	sendlength = GetIniFileString("SMSCommConfig", "length", sret, "smsconfig.ini");
