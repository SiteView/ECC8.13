...
MonitorExecutant::MonitorExecutant(void)
: m_pOption(NULL)
, m_StartTime(0)
, m_nRunCount(0)
, m_Monitor(NULL)
...
void MonitorExecutant::ExecuteMonitor()
{
	...
	{
		strError.Format("CoInitialize failed when create ");
	...
	try{
		...
		{
			char errlog[2048]={0};
			sprintf(errlog,"%s,MonitorID:%s",e.GetDescription(),m_Monitor->GetMonitorID());
		...
		AdjustRetByMonitor(m_Monitor, FALSE);
		m_Monitor->SetRunning(FALSE);
		CheckTaskQueueByMonitor(m_Monitor);

		//InitData();
	}catch(...)
	{
		char errlog[2048]={0};
		sprintf(errlog,"exit by exception");
		putil->ErrorLog(errlog);
		return;
...
bool MonitorExecutant::InitSocket(void)
{
#ifdef 	WIN32
	int err=0;
...
void MonitorExecutant::RunMonitor(void)
{
	m_nRunCount++;
	...
	if(m_pOption->m_isDemo)
	{
		puts("=================Demo mode=========================");
		strDLLName.Format("%s/fcgi-bin/%s",(char *)g_strRootPath,m_pOption->m_DemoDLL.c_str());
		strFuncName=m_pOption->m_DemoFunction.c_str();
...
BOOL MonitorExecutant::Init(void)
{
	...
	m_pTaskQueueManager=new QueuesManager(this);
...
void MonitorExecutant::SetMonitor(Monitor * pMonitor)
{
	m_Monitor=pMonitor;
}

int MonitorExecutant::MakeInBuf(void)
{
    CStringList&lstParam=m_Monitor->GetParameterList();
...
bool MonitorExecutant::RunInProcess(int iLen, string dllname, string funname)
{
	if(m_Monitor==NULL)
...
BOOL MonitorExecutant::PaserResultV70(void)
{
	m_RVmap.clear();
...
void MonitorExecutant::ParserMonitorState(void)
{
	if(m_MonitorState==Monitor::STATUS_BAD)
		return;
...
void MonitorExecutant::ProcessResultV70(void)
{
	ReturnValue* pRV=NULL;
...
//	puts(ps);
	...
	if(m_Monitor->m_isRefresh)
	{
		...
		//ofstream fout("monitor.log",ios::app);
		//fout <<"monitorcall:"<< g_RefreshQueueName<<","<<g_QueueAddr <<"\r\n"; 
		//fout << flush; 
		//fout.close();

		if(!::PushMessage(g_RefreshQueueName,"Refresh_OK",buf,strlen(buf)+1,"default",g_QueueAddr))
			putil->ErrorLog("PushMessage failed!");
		else
			puts("PushMessage succed!");
	}
}

void MonitorExecutant::ClearResult(void)
{
...
bool MonitorExecutant::BulidStringMap(STRINGMAP & map, const char * buf)
{
	if(buf==NULL)
		return false;
...
BOOL MonitorExecutant::CheckStateByType(int Type)
{
...
int MonitorExecutant::ParserExpression(CString strExpression, CStringList & lstOperator, CStringList & lstID)
{
...
BOOL MonitorExecutant::CheckSingleItemState(StateCondition *pSt , int ItemID)
{
	if(pSt==NULL) return FALSE;
	if(ItemID<=0)
		return FALSE;
...
BOOL MonitorExecutant::Judge(const char *szSource, const char *szDestination, const char *szRelation)
{
...
//调整并发控制
void MonitorExecutant::AdjustRetByMonitor(Monitor *pMonitor, bool isA)
{
...
BOOL MonitorExecutant::CheckTaskQueueByMonitor(Monitor *pMonitor)
{
	Monitor *pTa=NULL;
	if(pMonitor->GetQueueIndex()>=0)
...
bool MonitorExecutant::LoadPreLibrary(void)
{
...
bool MonitorExecutant::CheckSubsequent(Monitor * pMonitor, bool & bTotal, bool & bPer)
{
	bTotal=false;
	bPer=false;
	...
	if(pItem==NULL)
	{
		Subsequent *pSub = GetSubsequtent();
	...
	if(pEntity==NULL)
	{
		...
		pEntity = GetGroups()->GetEntityByID(eid.c_str());
		if(pEntity==NULL)
			return false;
