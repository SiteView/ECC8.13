...
{
	if(!m_pExecutant)
		throw MSException("Error : m_pExecutant is NULL when QueueManager initialization");
	CreateMainQueue();
	CreateQueueBySubsequent();

	Groups *pG=m_pExecutant->GetGroups();
...
{
	Subsequent *pSub=m_pExecutant->GetSubsequtent();
	if(!pSub)
		return FALSE;
