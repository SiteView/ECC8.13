...
BOOL RunErlangNode(PROCESS_INFORMATION*pi);
...
LOOP:
	 ...
	 if(g_SubProcessState&&(!g_Disabled))
	 {
		 ...
		 for(int i=0;i<6;i++)
		 {
			 ...
			 if(hd)
			 {
				 AddToEventLog("启动关键线程成功");
				 ::CloseHandle(hd);
				 break;
			 }
			 else if(hd==NULL&&i==5)
			 {
				 AddToErrorLog("启动关键线程失败!");
				 break;
			 }
		 }
	 }else
...
DWORD WINAPI ErlangThreadFunc( LPVOID lpParam )
{
	 ZeroMemory(&g_erlpi,sizeof(PROCESS_INFORMATION));

	 if(!RunErlangNode(&g_erlpi))
	 {
		 AddToErrorLog("Run Erlang Node process failed");
		 return 1;
	 }

	 ::WaitForSingleObject(g_erlpi.hProcess,INFINITE);
	 ::ResetEvent(g_hExit);

	 ::Sleep(2000);

	 ::CloseHandle(g_erlpi.hProcess);
	 ::CloseHandle(g_erlpi.hThread);
...
LOOP:
	 ...
	 if(g_SubProcessState&&(!g_Disabled))
	 {
		 ...
		 for(int i=0;i<6;i++)
		 {
			 HANDLE hd=::CreateThread(NULL,0,&ErlangThreadFunc,NULL,0,&dw);
			 if(hd)
			 {
				 AddToEventLog("启动Elang节点线程成功");
				 ::CloseHandle(hd);
				 break;
			 }
			 else if(hd==NULL&&i==5)
			 {
				 AddToErrorLog("启动Elang节点线程失败!");
				 break;
			 }
		 }
	 }else
...
BOOL Run(void)
{
	...
	if(g_bool_watch_Monitorshedule)
	{
		...
		for(i=0;i<6;i++)
		{
			HANDLE ht=::CreateThread(NULL,0,&ErlangThreadFunc,NULL,0,&dw);
			if(ht)
			{
				AddToEventLog("Start watch ErlangNode thread succeed");
				::CloseHandle(ht);
				break;
			}
			else if(ht==NULL&&i==5)
			{
				AddToErrorLog("Start watch ErlangNode thread failed");
				return FALSE;
			}
		}
...
BOOL RunErlangNode(PROCESS_INFORMATION*pi)
{
  ...
  //  PROCESS_INFORMATION pi;
	...
	strCommandLine="\"C:\\Program Files\\erl5.6.1\\bin\\erl\" -noshell -sname e1 -setcookie dragonflowecc -s eccsrv start"; 
	printf("commandline:%s\n",(LPCSTR)strCommandLine);
