...
//	pM->SetParentID( pid.c_str() );
	...
	{ 
		int pos=0;
		if( (pos=getvalue.find("true"))>-1 )
			pM->SetDisable(true);
		else if( (pos=getvalue.find("time"))>-1)
		{
...
//	typedef pair< svutil::word, Task* >  MAPPAIR;
	...
	if ( ! GetAllTaskName( tname, svdbuser, svdbaddr  ) )  
		return false;

	for(it=tname.begin(); it!=tname.end(); ++it)
	{
		OBJECT obj1=GetTask( *it, svdbuser, svdbaddr );  
		if (obj1==INVALID_VALUE ) 
			continue;
...
//		tmap.insert( MAPPAIR(*it,tasktemp) );
		tmap.insert(*it,tasktemp);

		string value; 	char key[128]={0};
		sprintf(key,"Type");
		...
		vector <HOUR_MIN> starttime,endtime;  vector <HOUR_MIN>::iterator hit;
		...
		for(int day=0; day<=6; ++day)
		{
			...
			for(hit=starttime.begin(); hit!=starttime.end(); ++hit)
			{
				...
				tasktemp->m_week[day].m_task.push_back(timetemp);
				++count;
			}
		} //end of for (day)
