...
const char *str_disable="¼à²âÆ÷±»½ûÖ¹";
const char *str_gddisable="¼à²âÆ÷±»×éÒÀ¿¿½ûÖ¹ËùÒÀ¿¿¼à²âÆ÷ID:%s";
const char *str_taskplandisable="¼à²âÆ÷±»ÈÎÎñ¼Æ»®½ûÖ¹,ÈÎÎñ¼Æ»®Ãû³Æ:%s";
const char *str_tempdisable="¼à²âÆ÷±»¼àÊ±½ûÖ¹,ÆðÊ¼Ê±¼ä:%s,ÖÕÖ¹Ê±¼ä:%s";
...
void ScheduleThread::run()
{
	...
	curTime=CTime::GetCurrentTimeEx();                   //ï¿½ï¿½ï¿?
	//::Sleep((61-curTime.GetSecond())*1000);
	Thread::sleep((61-curTime.GetSecond())*1000);

	//	::SetThreadPriority(this->m_hThread,THREAD_PRIORITY_HIGHEST);  //ï¿½ï¿½ß³ÌµÈ¼
			...
			while(it2!=MonitorList.end())
			{
				...
				if(pMonitor->GetFrequency()==nFrequency)
				{
					if(++n>nFrequency)
						n=1;
					temtime=time;
					ts=CTimeSpan(0,0,n%nFrequency,0);
					temtime+=ts;
					temtime-=FreSpan;
