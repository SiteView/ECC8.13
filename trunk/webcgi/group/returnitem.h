...
private:
    ...
    {
        FindNodeValue(node, "sv_name", m_szName);                    // 名称
        FindNodeValue(node, "sv_type", m_szType);                    // 类型

        if(FindNodeValue(node, "sv_label", m_szLabel))              // 标签
            m_szLabel = SVResString::getResString(m_szLabel.c_str());

        FindNodeValue(node, "sv_unit", m_szUnit);          // 是否允许为空
    };
    string m_szName;
    string m_szType;
    string m_szUnit;
