/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 文件: addmonitor2nd.h
// 添加或者修改监测器
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
#ifndef _SV_ADD_MONITOR_H_
#define _SV_ADD_MONITOR_H_
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma once
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
#include "../../opens/libwt/WTable"
#include "../../opens/libwt/WTableCell"
#include "../../opens/libwt/WPushButton"
#include "../../opens/libwt/WTextArea"
#include "../../opens/libwt/WComboBox"
#include "../../opens/libwt/WCheckBox"
#include "../../opens/libwt/WLineEdit"
#include "../../opens/libwt/WText"
class WImage;
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
#include <string>
#include <list>
#include <map>

using namespace std;

/////////////////////////////////////////////////////////////////////////////////////////////////////////////
#include "../../kennel/svdb/svapi/svapi.h"
#include "showtable.h"

#include "paramitem.h"
#include "returnitem.h"

#include "conditionparam.h"
#include "batchadd.h"

#include "../userright/user.h"
#include "basefunc.h"
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
class SVMonitor : public WTable
{
    //MOC: W_OBJECT SVMonitor:WTable
    W_OBJECT;
public:
    friend class SVGroupview;
    SVMonitor(WContainerWidget * parent = NULL, CUser * pUser = NULL, string szIDCUser = "default", 
        string szPwd = "localhost", int nMonitorID = -1);

    void showMonitorParam(int &nMTID,string &szDeviceIndex);                    // 显示监测器 参数                  

    void EditMonitorByID(string &szEditMonitorID);                              // 根据监测索引修改监测器

    void SetUserPwd(string &szUser, string &szPwd);                             // 设置IDC用户 密码

    string getCurrentEditID() {return m_szEditMonitorID;};
    string getParentID() {return m_szDeviceIndex;};

    void requesDynData();
public signals:
    //MOC: SIGNAL SVMonitor::CancelAddMonitor()
    void CancelAddMonitor();                                                    // 取消添加
    //MOC: SIGNAL SVMonitor::BackPreview()
    void BackPreview();                                                         // 返回上一级页面
    //MOC: SIGNAL SVMonitor::AddMonitorSucc(string,string)
    void AddMonitorSucc(string szName, string szIndex);                         // 添加成功
    //MOC: SIGNAL SVMonitor::EditMonitorSucc(string,string)
    void EditMonitorSucc(string,string);                                        // 编辑成功
    //MOC: SIGNAL SVMonitor::BatchAddNew()
    void BatchAddNew();                                        // 编辑成功
private slots:
    //MOC: SLOT SVMonitor::Preview()
    void Preview();                                                             // 上一级页面
    //MOC: SLOT SVMonitor::showHelpText()
    void showHelpText();                                                        // 显示/隐藏帮助
    //MOC: SLOT SVMonitor::SaveMonitor()
    void SaveMonitor();                                                         // 保存监测器
    //MOC: SLOT SVMonitor::Cancel()
    void Cancel();                                                              // 取消
    //MOC: SLOT SVMonitor::setDefaultAlert()
    void setDefaultAlert();
    //MOC: SLOT SVMonitor::ContinueAdd()
    void ContinueAdd();
    //MOC: SLOT SVMonitor::BatchAddMonitor()
    void BatchAddMonitor();
    //MOC: SLOT SVMonitor::listDynParam()
    void listDynParam();
	//MOC: SLOT SVMonitor::Translate()
	void Translate();                                                           // 翻译
private:
    int m_nMonitorID;                                                           // 监测器模板的索引
    string m_szMonitorName;                                                     // 监测器模版的显示名称
    string m_szDeviceIndex;                                                     // 待添加监测器的设备
    string m_szNetworkset;
    string m_szEditMonitorID;                                                   // 编辑监测器索引
    string m_szHostName;                                                        // 主机名称
    string m_szPoint;
    string m_szDll;
    string m_szFunc;
    string m_szQuery;

    bool   m_bHasDynamic;
    bool   m_bContinueAdd;
    bool   m_bError;
    OBJECT m_objTemplate;                                                       // 监测器模板类

    list<SVParamItem*> m_lsDyn;

    typedef map<string, SVParamItem* , less<string> >::iterator mapitem;
    typedef list<SVParamItem*>::iterator listItem;
    list<SVParamItem*> m_lsBaseParam;                                           // 基础数据
    list<SVParamItem*> m_lsAdvParam;                                            // 高级

    list<SVReturnItem*> m_lsReturn;                                             // 返回值 list

    SVShowTable        * m_pGeneral;                                            // 基础选项
    SVShowTable        * m_pAdv;                                                // 高级选项
    SVShowTable        * m_pCondition;                                          // 条件

    bool m_bShowHelp;                                                           // 显示/隐藏帮助
private: // fuction
    void initForm();                                                            // 初始化页面

    void createTitle();                                                         // 标题
    void createHelp();                                                          // 帮助按钮
    void createAdvParam();                                                      // 创建高级选项
    void createBaseAdv(WTable * pTable);                                        // 创建高级参数中的基础数据

    void createDeviceName(WTable * pTable);                                     // 设备名称
    void createMonitorName(WTable * pTable);                                    // 监测器名称
    void createBaseParam();                                                     // 基础选项 
    void createConditionParam();                                                // 条件
    void createOperate();                                                       // 操作

    void enumBaseParam();                                                       // 枚举基础选项
    void enumAdvParam();                                                        // 枚举高级选项
    void enumReturnParam();                                                     // 枚举返回值

    void enumDeviceRunParam();

    //void loadStrings();                                                         // 加载字符串
    void clearData();                                                           // 清理数据
    void getHostNameByID(string &szDeviceID, string &szHostName);               // 根据设备ID得到主机名称

    void checkBaseParam();                                                      // 校验基础数据
    void checkAdvParam();                                                       // 校验高级数据
    void checkConditionParam();                                                 // 校验报警条件
    void checkMonitorName();
    void checkMonitorFreq();

    bool saveBaseAdvParam(MAPNODE &mainnode);
    bool saveBaseParam(OBJECT &objMonitor);                                     // 保存基础数据
    bool saveAdvParam(OBJECT &objMonitor);                                      // 保存高级数据
    bool saveCondition(OBJECT &objMonitor);                                     // 保存报警条件

    void SetBaseAdvParam(MAPNODE &mainnode);
    void SetBaseParam(OBJECT &objMonitor);                                      // 设置基础数据
    void SetAdvParam(OBJECT &objMonitor);                                       // 设置高级数据
    void SetConditionParam(OBJECT &objMonitor);                                 // 设置报警条件

    void clearBaseParam();                                                      // 清理基础数据
    void clearAdvParam();                                                       // 清理高级选项
    void clearConditionParam();                                                 // 清理报警条件

    //string m_szBack;                                // 返回
    //string m_szBackTip;                             // 返回上一级提示
    //string m_szAdd;                                 // 添加
    //string m_szAddTip;                              // 添加提示
    //string m_szCancel;                              // 取消
    //string m_szCancelTip;                           // 取消提示
    //string m_szSaveTip;                             // 保存提示
    //string m_szSave;                                // 保存
    //string m_szBatchAdd;                            // 批量添加
    //string m_szBatchAddTip;                         // 批量添加提示
    //string m_szContinueAdd;                         // 继续添加
    //string m_szContinueAddTip;                      // 继续添加提示
    //string m_szSetAlert;
    //string m_szSetAlertTip;

    //string m_szMonitor;                             // 监测器
    //string m_szAddTitle;                            // 添加标题
    //string m_szEditTitle;                           // 编辑标题
    //string m_szAdvTitle;                            // 高级选项标题
    //string m_szConditionTitle;                      // 报警条件标题
    //string m_szGeneralTitle;                        // 基础选项标题
    //string m_szHelpTip;
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    struct BASE_ADV_PARAM
    {
        BASE_ADV_PARAM()
        {
            m_pDescription = NULL;                   // 监测器描述
            m_pReportDesc = NULL;                    // 报告描述
            m_pPlan = NULL;                          // 任务计划
            m_pCheckErr = NULL;                      // 校验错误
            m_pMonitorFreq = NULL;                   // 错误时监测频率
            m_pTimeUnit = NULL;                      // 时间单位
        }
        WTextArea * m_pDescription;                   // 监测器描述
        WTextArea * m_pReportDesc;                    // 报告描述
        WComboBox * m_pPlan;                          // 任务计划
        WCheckBox * m_pCheckErr;                      // 校验错误
        WLineEdit * m_pMonitorFreq;                   // 错误时监测频率
        WComboBox * m_pTimeUnit;                      // 时间单位
    };
    //struct BASE_ADV_SHOW_TEXT
    //{
    //    string m_szDesc, m_szDescHelp;                  // 描述， 描述帮助
    //    string m_szCheckErr, m_szCheckHelp;             // 校验错误，校验错误帮助
    //    string m_szPlan, m_szPlanHelp;                  // 任务计划，任务计划帮助
    //    string m_szFreq, m_szFreqHelp;                  // 监测频率，监测频率帮助
    //    string m_szReport, m_szReportHelp;              // 报告提示信息，报告帮助
    //    string m_szDevice, m_szDeviceHelp;              // 设备名称，设备名称帮助
    //    string m_szMonitor, m_szMonitorHelp;            // 监测器名称，监测器名称帮助
    //    string m_szErrName, m_szFreqErr;//, m_szErrIPAddress;        // 监测器名称错误
    //};

    //// 时间单位
    //struct TIME_UNIT
    //{
    //    string      m_szMinute;         // 分钟
    //    string      m_szHours;          // 小时
    //};
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //BASE_ADV_SHOW_TEXT m_ShowText;
    BASE_ADV_PARAM     m_AdvList;
    //TIME_UNIT          m_timeunit;

    map<WText*, string , less<WText*> > m_HelpMap;              
    map<WText*, string , less<WText*> >::iterator m_helpItem;
    WText         *    m_pDeviceName;                           // 设备名称
    WLineEdit     *    m_pMonitorName;                          // 监测器名称

    WPushButton   *    m_pBack;                                 // 返回
    WPushButton   *    m_pCancel;
    WPushButton   *    m_pSave;                                 // 添加
    WPushButton   *    m_pBatchAdd;                             // 批量添加
    WPushButton   *    m_pContinue;                             // 继续按钮

    WPushButton   *    m_pSetAlert;
    WPushButton   *    m_pListDynBtn;

    WText         *    m_pMonitorHelp;                          // 监测器名称标题
    WText         *    m_pTitle;                                // 标题控件
    WText         *    m_pFreqHelp;

    SVConditionParam * m_pErrCond;                              // 错误条件
    SVConditionParam * m_pWarnCond;                             // 警告条件
    SVConditionParam * m_pGoodCond;                             // 正常条件

    WTable           * m_pContentTable;
    WTable           * m_pSubContent;
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    string         m_szIDCUser;
    string         m_szIDCPwd;
    CUser        * m_pSVUser;
    
	//翻译
	WPushButton * m_pTranslateBtn;
	
	//string m_szTranslate;
	//string m_szTranslateTip;

	//写操作日志
	//string m_szTypeAdd;
};

#endif
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// end file
