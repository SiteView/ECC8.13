...
//map<string, string, less<string> > SVResString::m_lsResource;
...
void SVResString::loadStrings()
{
    ...
    if(m_objRes != INVALID_VALUE)
    {
        ...
        if(resNode != INVALID_VALUE)
        {
            loadAddSVSEStrings(resNode);               // 加载添加SE资源字符串
            loadAddGroupStrings(resNode);              // 加载添加组资源字符串
            loadAddDeviceStrings(resNode);             // 加载添加设备资源字符串
            loadAddMonitorStrings(resNode);            // 加载添加监测器资源字符串
            loadBatchAddStrings(resNode);              // 加载批量添加资源字符串
            loadConditionStrings(resNode);             // 加载报警条件资源字符串
            loadTreeviewStrings(resNode);              // 加载树显示资源字符串
            loadSVSEViewStrings(resNode);              // 加载SE 视图显示资源字符串
            loadGroupviewStrings(resNode);             // 加载组视图显示资源字符串
            loadMonitorviewStrings(resNode);           // 加载监测器视图显示资源字符串
            loadSortStrings(resNode);                  // 加载排序操作资源字符串
            loadGeneralStrings(resNode);               // 加载通用资源字符串
        }
        //CloseResource(objRes);
    }
}
...
void SVResString::loadGeneralStrings(MAPNODE &objNode)
{
    string szTranslate(""), szTranslateTip(""), szEdit(""), szDevice(""),
        ...
        szErr(""), szNormal(""), szWarn(""), szAffir(""), szPoint(""), szRefresh(""), szRefeshTip("");
    ...
    FindNodeValue(objNode, "IDS_Refresh_Tip",           szRefeshTip);           // 刷新
    ...
    m_lsResource["IDS_Refresh_Tip"] = szRefeshTip;
}
...
void SVResString::loadGroupviewStrings(MAPNODE &objNode)
{
    ...
    FindNodeValue(objNode, "IDS_State_Description",         szColDesc);                 // 状体描述
