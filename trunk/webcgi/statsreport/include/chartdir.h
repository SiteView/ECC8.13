/*
 * Copyright (C) 2005 Advanced Software Engineering Limited.
 *
...
inline void OUT_DEBUG(int argc, char * argv[], const char * pszTitle = "")
{
	std::string strDbg;
	strDbg = pszTitle;
	strDbg += ": ";
	...
	char szDbg[256] = {0};
	::sprintf(szDbg, "pid:%d; tid:%d; ", ::GetCurrentProcessId(), ::GetCurrentThreadId());
	strDbg += szDbg;
	strDbg += "for debug";
...
namespace Chart 
{
	...
	static inline int silverColor(int angle = 90) { return metalColor(0xcccccc, angle); }
	...
	enum { DiamondPointer, TriangularPointer, ArrowPointer, 
		ArrowPointer2, LinePointer, PencilPointer };
...
public :
	...
	void setExplode(int sectorNo, int distance = -1)
	...
	{ Sector *ret = new Sector(CPieChart_sector(ptr, sectorNo)); reg(ret); return ret; }
};
...
public :
	...
	void setPos(int x, int y, int align = Chart::Center)
	{ CAxis_setPos(ptr, x, y, align); }
...
public :
	...
	{ DataSet *ret = new DataSet(CLayer_getDataSet(ptr, dataSet)); reg(ret); return ret;}
