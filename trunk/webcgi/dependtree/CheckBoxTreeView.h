...
static inline const char* findIndexByRealID(string &szRealID)
{
    const char* pPos = strrchr(szRealID.c_str(), '.');
    if(pPos && *(pPos + 1) != '\0')
        return pPos + 1;
    else
        return "";
...
public:
    ...
    bool InitTree(std::string strFirstNode_="", bool bDevice_ =true,bool bCheck_ =true,bool bMain_=false,
        std::string strUser_="admin", std::string szSEID = "1");
