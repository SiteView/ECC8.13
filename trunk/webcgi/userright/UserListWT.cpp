...
WSlot_ CUserList::slots_[] = { WSlot_("AddUser()", false), WSlot_("BeforeDelUser()", false), WSlot_("DelUser()", false), WSlot_("SelAll()", false), WSlot_("EditUser(const std::string)", false), WSlot_("EditAdminUser(const std::string)", false), WSlot_("SelNone()", false), WSlot_("SelInvert()", false), WSlot_("SaveUser()", false), WSlot_("CancelUser()", false), WSlot_("Translate()", false), WSlot_("ExChange()", false), WSlot_("ShowHelp()", false), WSlot_() };
...
bool CUserList::triggerSlot(WObject *sender, const WSlot_ *slot, void **args)
{
  ...
  if (slot == slots_ + 5) {
    sender_ = sender;    EditAdminUser(*((const std::string *)args[0]));
    return true;
  }
  if (slot == slots_ + 6) {
