...
// start
/*
CTaskList::CTaskList(WContainerWidget * parent, WTable * table):
WTable(parent)
{
    AddItem(table);
}
...
CTaskList::CTaskList(WContainerWidget * parent, WSVFlexTable * table,const string helpSun):
WTable(parent)
{
		...
		m_pEnd[0]->resize(WLength(100, WLength::Pixel),WLength(20, WLength::Pixel));
		...
		for (int i=1; i<7; i++)
		{
			...
			m_pEnd[i]->resize(WLength(100, WLength::Pixel),WLength(20, WLength::Pixel));
			m_pEnd[i]->setStyleClass("input_text");
		}
		
}

void CTaskList::AddItem(WTable * parent)
{
	string szMon,szThurs,szWed,szTues,szFri,szSat,szSun,szDisable,szEnable,szfrom,szTo;
	//Resource
	OBJECT objRes=LoadResource("default", "localhost");  
    ...
    for(int i = 0; i < 7 ; i ++)
    {
		...
		pFrom[i] = new WText(szfrom, (WContainerWidget*)parent -> elementAt(2*i+3 , 1));
	
		m_pStart[i] = new WLineEdit("00:00", (WContainerWidget*)parent -> elementAt(2*i+3 , 1));
		m_pStart[i] -> setTextSize(30);

		pTo[i] = new WText(szTo, (WContainerWidget*)parent -> elementAt(2*i+3 , 1));
	
		m_pEnd[i] = new WLineEdit("23:59", (WContainerWidget*)parent -> elementAt(2*i+3 , 1)); 
		m_pEnd[i] -> setTextSize(30);
