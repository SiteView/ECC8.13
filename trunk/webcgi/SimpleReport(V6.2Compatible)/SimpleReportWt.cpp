...
WSlot_ CSimpleReport::slots_[] = { WSlot_("Translate()", false), WSlot_("ExChange()", false), WSlot_("AddNum()", false), WSlot_("InitNum()", false), WSlot_() };
...
bool CSimpleReport::triggerSlot(WObject *sender, const WSlot_ *slot, void **args)
{
  ...
  if (slot == slots_ + 1) {
	...
	if (slot == slots_ + 2) {
		sender_ = sender;    AddNum();
		return true;
	}
	if (slot == slots_ + 3) {
		sender_ = sender;    InitNum();
		return true;
	}
