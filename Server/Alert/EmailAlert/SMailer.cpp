...
void MailSender::GetMailBaseInfo(const std::string server_name, 
               const std::string user_name , 
               const std::string user_pwd )
{
	_server_name = server_name;
...
void MailSender::conn()
{
    ...
    if (host == 0)
    {
		printf("Get server infomation error\n");
    ...
    server_addr.sin_port = htons(SERVICE_PORT);
	 ...
	 long iTimeOut=10*1000;                                                 //20s   
	//接收超时设定   
	::setsockopt(_socket,SOL_SOCKET,SO_RCVTIMEO,(char   *)&iTimeOut,sizeof(iTimeOut));     
	//发送超时设定   
	::setsockopt(_socket,SOL_SOCKET,SO_SNDTIMEO,(char   *)&iTimeOut,sizeof(iTimeOut));   

	linger lin;
	lin.l_linger = 0;
	lin.l_onoff = 1;
	int iOptLen = sizeof lin;
	if (setsockopt(_socket, SOL_SOCKET, SO_LINGER, (char*)&lin, iOptLen) == SOCKET_ERROR)
	{
		_socket = INVALID_SOCKET;
		throw new std::exception("set socket option err");
	}
	else
	{
...
void MailSender::login()
{
	...
	*/
	...
	sendRequest(send_auth_cmd, msg);
	rcvResponse("334");
	string pwd = Base64Helper::encode(_user_pwd);
	msg = pwd+"=\r\n";
	sendRequest(send_auth_cmd, msg);
	rcvResponse("235");
...
bool MailSender::rcvResponse(const std::string expected_response)
{
	...
	response_buffer[0]='\0';
	recv_bytes = recv(_socket, response_buffer, MAX_BUFFER_SIZE, 0);
	std::cout << "[INFO]RECEIVE:" << response_buffer << std::endl;
    if ( recv_bytes < 0 )
    {
       throw MailException(ErrorMessage::getInstance().response(expected_response));
		return false;
                             ...
                             const std::string content)
{
	...
	int nRet=send(_socket, content.c_str(), content.length(), 0);
    if ( nRet < 0 )
    {
