...
int CSerialPort::page(char *SrcMsg, MsgList *DestMsg)
{
	...
	else
	{
		...
		while(strlen(SrcMsg)>=MaxSMSLen)
		{
			...
			{
                //拷贝最大短信长度-1
				strncpy(msgTemp->chMsg,SrcMsg,MaxSMSLen-1);
                //添加结束符号
				msgTemp->chMsg[MaxSMSLen-1]='\0';
                //原始字符串为从0开始减去最大短信长度-1剩余的字符串
				strcpy(SrcMsg , SrcMsg+MaxSMSLen-1);
			}
			else
			{
				strncpy(msgTemp->chMsg,SrcMsg,MaxSMSLen-2);
				msgTemp->chMsg[MaxSMSLen-2]='\0';	
				strcpy(SrcMsg , SrcMsg+MaxSMSLen-2);
			}
