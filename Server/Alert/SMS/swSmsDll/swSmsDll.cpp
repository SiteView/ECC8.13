...
int process_msg(std::vector<std::string> &vOut,string strIn)
{
	string strMsg = strIn;
	...
	if (strMsg.size() > g_msglen+4)
	{
		...
		for(int i=0; i < vTmp.size(); i++)
		{
			...
			sprintf(szTmp, "<1>%d</1><2>%d</2><3>%s</3>",vTmp.size(),i+1,vTmp[i].c_str());
...
int SendSms(char *szSMTo, char *szContent)
{
	...
	try
	{
		...
		for (std::vector<std::string>::iterator i = vMobiles.begin(); i != vMobiles.end(); i++)
		{
			...
			{
				otl_stream s(1,"begin SWYD.KF_P_SMS_SEND_FOR_DYH(:mobile<varchar,in>,:msg<varchar,in>,:sendid<varchar,out>);end;",db);

				s.set_commit(0);

				otl_long_string sendid;
