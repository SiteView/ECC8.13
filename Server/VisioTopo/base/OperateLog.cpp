...
void OperateLog::OperateObjNameInit()
{
    string strObjName[18] = {"", "", "", "", "",
							 ...
							 "", "", ""};
	//Resource
	string keys="IDS_System,\
				,IDS_SE,\
				,IDS_Group,\
				,IDS_Device,\
				,IDS_Monitor_Title,\
				,IDS_General_MainTitle,\
				,IDS_Customer_Name,\
				,IDS_Monitor_Server_Name,\
				,IDS_Alert_Rule,\
				,IDS_Tuopu,\
				,IDS_SysLogSetting,\
				,IDS_User_Manage,\
				,IDS_Email_Set,\
				,IDS_Time_Area_Task_Plan,\
				,IDS_Time_Task_Plan,\
				,IDS_SMS_Config,\
				,IDS_TopN_Report,\
				,IDS_Total_Report,\
...
{	
	//Resource
	OBJECT objRes=LoadResourceByKeys("IDS_All","default", "localhost"); 
	if( objRes !=INVALID_VALUE )
	{	
		MAPNODE ResNode=GetResourceNode(objRes);
		if( ResNode != INVALID_VALUE )
...
//
bool OperateLog::IsMyOperateLog(string strUseIndex)
{
	//1、获取当前IDCID，　根据_UserID从idcuser.ini和idcchilduser.ini 读取IDCID来做比较和查询idcuser.ini

	bool bFlag = false;

	string strTmpIdcId = GetIniFileString_2(strUseIndex, "IdcUserId", "", "idcchilduser.ini", "localhost", "root");
	if(strTmpIdcId.compare(strIdcUserId) == 0)
	{
		bFlag = true;
	}

	strTmpIdcId = GetIniFileString_2(strUseIndex, "IdcUserId", "", "idcuser.ini", "localhost", "root");
	if(strTmpIdcId.compare(strIdcUserId) == 0)
	{
		bFlag = true;
	}
...
bool OperateLog::QueryOperateRecord(string strUserName, string strOObj, TTime startTime, TTime endTime, string strOType, list<OperateLogItem> &RecordsList)
{
	...
	while((rdobj=::FindNextRecord(item))!=INVALID_VALUE)
	{
		...
		//	{
		//		string strTemp = GetIniFileString_2(strRecordValue, "LoginName", "", "user.ini");

		//	#ifdef IDC_Version	
		//		if(IsMyOperateLog(strRecordValue))
		//	#endif
		//		{
		//			//如果是IDC版只能查询此IDC用户的操作日志
